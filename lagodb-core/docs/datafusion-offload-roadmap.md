# DataFusion Offload Roadmap (LagoDB core CustomScan extension)
...
design is based on the current `lagodb-core` CustomScan filter-pushdown
...
## Current LagoDB baseline

`lagodb-core` currently has one pushdown layer:
...
For LagoDB, the equivalent of ParadeDB's Tantivy fast fields is not an
...
`lagodb-core` needs a second CustomScan framework beside the current
...
```rust
trait LagodbDataFusionProvider {
