...
enum ProviderBootstrapError {
    ...
    #[error("pg_lakebase_runtime cannot be configured as its own provider library")]
    RuntimeLibrary,
    #[error("provider library {name:?} did not register a Lakebase provider")]
...
pub(crate) fn init() {
    GucRegistry::define_string_guc(
        c"pg_lakebase.provider_libraries",
        c"Lakebase AM and FDW provider libraries",
        c"Comma-separated PostgreSQL library basenames loaded once by pg_lakebase_runtime during shared preload. Restart required.",
        &PROVIDER_LIBRARIES,
...
pub(crate) fn load_configured() {
    ...
    let libraries = parse_library_names(&configured).unwrap_or_else(|error| {
        panic!("invalid pg_lakebase.provider_libraries: {error}")
    });
    for library in libraries {
        ...
        if registration_count == 0 {
            ...
            panic!("cannot bootstrap Lakebase provider: {error}");
        }
    }
}
