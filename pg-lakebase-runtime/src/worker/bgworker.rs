    ...
    fn new() -> Self {
        ...
        let raw = unsafe {
            pg_sys::AllocSetContextCreateExtended(
                parent,
                c"pg_lakebase dynamic worker registration".as_ptr(),
...
impl DynamicWorkerRegistration {
    ...
    pub(super) fn register_coordinator(
        ...
        let name = format!("pg-lakebase coordinator db={database_oid}");
        Self::register(
    ...
    pub(super) fn register_worker(
        ...
        let name = format!(
            "pg-lakebase worker db={} id={}",
            key.database_oid, key.worker_id,
        );
