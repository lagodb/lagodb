    ...
    fn run_worker_entrypoint(store: &Store, key: WorkerKey, start: WorkerStart) {
        ...
        let function = WorkerTransaction::run(|| -> Result<_, PgReportError> {
            ...
            Spi::run("SELECT set_config('search_path', '', false)").map_err(
                |source| {
                    PgReportError::from(LakebaseError::WorkerEntrypointPreparation {
                        source,
                    })
                },
            )?;
            ...
            match function {
                ...
                Ok(WorkerPreparation::RegistrationMissing) => {
                    diag::warning(format_args!(
                        "Lakebase worker registration disappeared before start: database_oid={database_oid}, extension_oid={extension_oid}, worker_name={worker_name}"
                    ));
                ...
                Err(error) => {
                    diag::warning(format_args!(
                        "failed to load Lakebase worker entry point: database_oid={database_oid}, extension_oid={extension_oid}, worker_name={worker_name}, error={error}"
                    ));
        ...
        diag::info(format_args!(
            "starting Lakebase extension worker: database={database_name}, database_oid={database_oid}, extension_oid={extension_oid}, worker_name={worker_name}"
        ));
        ...
        let schedule = match code {
            ...
            unsupported => {
                diag::warning(format_args!(
                    "Lakebase worker returned an unsupported negative restart delay: database_oid={database_oid}, extension_oid={extension_oid}, worker_name={worker_name}, delay={unsupported}"
                ));
                WorkerSchedule::Idle
            }
        };
