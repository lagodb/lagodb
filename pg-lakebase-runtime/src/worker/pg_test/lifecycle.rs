...
mod tests {
    ...
    fn supervisor_restart_preserves_worker_at_production_boundary() {
        ...
        let terminated = Spi::get_one::<bool>(
            "SELECT pg_catalog.pg_terminate_backend(pid) FROM lakebase.process_status WHERE process_kind = 'supervisor'",
        )
        ...
        harness.wait_for_query(
            ...
            &format!(
                concat!(
                    ...
                    "FROM lakebase.process_status WHERE process_kind = 'supervisor') ",
                    "AND (SELECT pid = {} AND process_state = 'running' ",
                    "FROM lakebase.worker_status WHERE database_oid = {} ",
                    ...
                    "WHERE backend_type = 'pg-lakebase worker' AND datid = {})"
                ),
                old_supervisor_pid,
                old_worker_pid,
    ...
    fn supervisor_and_coordinator_ignore_statement_cancel() {
        ...
        let unchanged = Spi::get_one::<bool>(&format!(
            concat!(
                "SELECT EXISTS (SELECT FROM lakebase.process_status ",
                "WHERE process_kind = 'supervisor' AND pid = {} ",
                "AND state = 'running') AND EXISTS (SELECT FROM ",
                "lakebase.process_status WHERE process_kind = 'coordinator' ",
