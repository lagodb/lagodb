...
use super::batch::{IcebergArrowBatchSource, RowLocationLayout, position_unchecked};
...
impl IcebergBatchCursor {
    pub(super) fn query(
        ...
        Self {
            kind: CursorKind::Query(QueryBatchCursor {
                source,
                decoder,
                current: None,
                row_index: 0,
    ...
    fn next_into_slot(&mut self, out: &mut SlotColumns<'_>) -> AmResult<bool> {
        loop {
                ...
                && self.row_index < self.decoder.num_rows(bound)
            {
                ...
                unsafe {
                    self.decoder.write_row_unchecked(bound, self.row_index, out)
                }?;
                self.row_index += 1;
                return Ok(true);
            }
