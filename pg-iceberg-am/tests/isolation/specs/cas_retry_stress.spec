# Advanced concurrent update test for metadata_tracking.rs commit_all()
# This test specifically targets the CAS retry loop (L336-L383)
# and tests various conflict scenarios
...
#    - Compatible with AccessShareLock (used by S2 during Rebase/Read)
#    - Conflicting with RowExclusiveLock (used by S2 during Update/Write)
# 2. S2 performs Rebase successfully (reading global V0) but BLOCKS when trying to Update.
...
#    - S2 reads tuple. Sees V0 (S1 uncommitted). Passes check L336.
#    - S2 attempts Update (L388). Blocks on S1's Tuple Lock.
# 4. S1 commits.
