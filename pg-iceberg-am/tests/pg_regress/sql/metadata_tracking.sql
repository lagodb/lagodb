...
SELECT metadata_location AS v0 FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass \gset
...
BEGIN;
  ...
  SELECT metadata_location AS v1 FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass \gset
  
  -- Inside transaction, catalog should still show v0 (no update yet)
  SELECT metadata_location = :'v0' as still_v0 FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass;
  ...
  SAVEPOINT sp1;
    ...
    SELECT * FROM test_meta ORDER BY id;
    SELECT metadata_location AS v2 FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass \gset
...
FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass;

-- Capture current state as v_base
SELECT metadata_location AS v_base FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass \gset
...
BEGIN;
  ...
  SELECT metadata_location AS v_new FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass \gset
...
SELECT metadata_location AS v_base2 FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass \gset
...
BEGIN;
  ...
  SELECT metadata_location AS v_l1_old FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass \gset
  
  SAVEPOINT sp1;
    INSERT INTO test_meta VALUES (7); -- v_l2
    SELECT metadata_location AS v_l2_old FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass \gset
...
SELECT metadata_location AS v2_initial FROM lakebase.iceberg_metadata WHERE relid = 'test_meta_2'::regclass \gset
SELECT metadata_location AS v1_initial FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass \gset
...
SELECT 
    (SELECT metadata_location != :'v1_initial' FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass) as t1_updated,
    (SELECT metadata_location != :'v2_initial' FROM lakebase.iceberg_metadata WHERE relid = 'test_meta_2'::regclass) as t2_updated;

--
-- Scenario 6: Main transaction rollback
--
SELECT metadata_location AS v1_before_rollback FROM lakebase.iceberg_metadata WHERE relid = 'test_meta'::regclass \gset
