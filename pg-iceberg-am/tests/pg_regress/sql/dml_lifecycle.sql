...
-- Matched MERGE branches currently reach AM update/delete/lock paths that
-- IcebergModify does not implement yet, but they must fail as unsupported
-- features rather than corrupting the frame stack.
...
SELECT 'merge update unsupported cleaned up' AS check;
SELECT * FROM dml_lifecycle.merge_update_t ORDER BY id;
INSERT INTO dml_lifecycle.merge_update_t VALUES (32, 'after_failed_merge');
SELECT * FROM dml_lifecycle.merge_update_t ORDER BY id;
...
SELECT 'merge delete unsupported cleaned up' AS check;
SELECT * FROM dml_lifecycle.merge_delete_t ORDER BY id;
INSERT INTO dml_lifecycle.merge_delete_t VALUES (42, 'after_failed_merge');
SELECT * FROM dml_lifecycle.merge_delete_t ORDER BY id;
