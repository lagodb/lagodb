...
BEGIN
    ...
    IF NOT rejected
       ...
       OR NOT EXISTS (
           SELECT FROM pg_extension WHERE extname = 'pg_iceberg_am'
       )
    THEN
...
BEGIN
    IF EXISTS (
           ...
           WHERE extname IN ('pg_lakebase_runtime', 'pg_iceberg_am')
...
SET client_min_messages = warning;
DROP EXTENSION IF EXISTS pg_iceberg_am;
RESET client_min_messages;
CREATE EXTENSION pg_iceberg_am;
