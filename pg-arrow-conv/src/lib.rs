...
//! - [`ColumnReader::bind`] resolves a batch column's concrete Arrow array once,
//!   then [`ColumnReader::read_datum`] / [`ColumnReader::read_cell`] read values
//!   (slot datum / `Cell`) without a per-value downcast;
