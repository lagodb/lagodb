...
impl ObjectCache {
    ...
    pub(crate) fn get_manifest(
        ...
        let cache_entry = self
            ...
            .try_get_with(key.clone(), || {
                self.fetch_and_parse_manifest(manifest_file)
            })
            .map_err(|err| {
                Error::new(
    ...
    /// or retrieves one from FileIO and parses it if not present
    pub(crate) fn get_manifest_list(
        &self,
        ...
        let cache_entry = self
            .cache
            .try_get_with(key.clone(), || {
                self.fetch_and_parse_manifest_list(snapshot, table_metadata)
            })
            .map_err(|err| {
    ...
    fn fetch_and_parse_manifest(
        ...
        let manifest = manifest_file.load_manifest(&self.file_io)?;

        Ok(CachedItem::Manifest(Arc::new(manifest)))
    }

    fn fetch_and_parse_manifest_list(
        &self,
        snapshot: &SnapshotRef,
        table_metadata: &TableMetadataRef,
    ) -> Result<CachedItem> {
...
mod tests {
    ...
    impl TableTestFixture {
        ...
        fn setup_manifest_files(&mut self) {
    ...
    #[test]
    fn test_get_manifest_list_and_manifest_from_disabled_cache() {
        ...
        let result_manifest = object_cache.get_manifest(manifest_file).unwrap();
    ...
    #[test]
    fn test_get_manifest_list_and_manifest_from_default_cache() {
        let mut fixture = TableTestFixture::new();
        fixture.setup_manifest_files();
