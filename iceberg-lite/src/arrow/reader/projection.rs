...
use arrow_schema::{FieldRef, Schema as ArrowSchema, SchemaRef as ArrowSchemaRef};
...
fn leaf_count(ty: &ParquetType) -> usize {
    if ty.is_primitive() {
        1
    } else {
...
/// Builds a field-id to leaf-column map from Arrow field-id metadata.
///
/// This is used for Parquet files without embedded field IDs when name mapping
/// has assigned IDs to the Arrow schema. Arrow and Parquet leaves use the same
/// depth-first order, so Arrow leaf indices line up with Parquet leaf columns.
fn build_field_id_map_from_arrow_schema(
    arrow_schema: &ArrowSchemaRef,
) -> HashMap<i32, usize> {
    let mut column_map = HashMap::new();
...
mod tests {
    ...
    use crate::spec::{
        DataFileFormat, Datum, MappedField, NameMapping, NestedField, PrimitiveType,
        Schema, Type,
    };
...
message schema {
  ...
  optional fixed_len_byte_array(17) c3 (DECIMAL(39,3)) = 3;
    ...
    #[test]
    fn test_read_parquet_with_name_mapping_uses_mapped_field_ids() {
        let schema = Arc::new(
            Schema::builder()
                .with_schema_id(1)
                .with_fields(vec![
        ...
        let file_path = format!("{table_location}/name_mapping.parquet");

        let name_col =
            Arc::new(StringArray::from(vec!["Alice", "Bob", "Charlie"])) as ArrayRef;
        let subdept_col =
            Arc::new(StringArray::from(vec!["comms", "tax", "audit"])) as ArrayRef;
        let batch =
            RecordBatch::try_new(arrow_schema.clone(), vec![name_col, subdept_col])
                .unwrap();
        ...
        assert_eq!(name_array.null_count(), 0);
    ...
    #[test]
    fn test_read_parquet_file_without_field_ids() {
        let schema = Arc::new(
            Schema::builder()
                .with_schema_id(1)
                .with_fields(vec![
    ...
    #[test]
    fn test_read_parquet_without_field_ids_partial_projection() {
        use arrow_array::Int32Array;
    ...
    #[test]
    fn test_read_parquet_without_field_ids_schema_evolution() {
        use arrow_array::{Array, Int32Array};
    ...
    #[test]
    fn test_read_parquet_without_field_ids_multiple_row_groups() {
        use arrow_array::Int32Array;
    ...
    #[test]
    fn test_read_parquet_without_field_ids_with_struct() {
    ...
    #[test]
    fn test_read_parquet_without_field_ids_schema_evolution_add_column_in_middle() {
        use arrow_array::{Array, Int32Array};
    ...
    #[test]
    fn test_read_parquet_without_field_ids_filter_eliminates_all_rows() {
    ...
    #[test]
    fn test_bucket_partitioning_reads_source_column_from_file() {
        use arrow_array::Int32Array;
