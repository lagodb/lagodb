...
impl ArrowReaderBuilder {
    ...
    ///
    /// This is retained for API parity with the upstream async reader. The sync
    /// Parquet `ChunkReader` path does not expose multi-range reads, so the
    /// value is not applied by iceberg-lite's synchronous reader.
    ...
    /// Parquet `ChunkReader` path does not expose multi-range reads, so the
    /// value is not applied by iceberg-lite's synchronous reader.
    pub fn with_range_fetch_concurrency(
        mut self,
