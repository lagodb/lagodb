...
/// Synchronous delete file loader with OnceLock-based caching.
///
/// This loader uses `OnceLock` internally (via `DeleteFilter`) to ensure:
/// - Each delete file is loaded exactly once
/// - Concurrent requests for the same file will block and wait for the first loader
/// - Results are cached and shared across all file scan tasks
#[derive(Clone, Debug)]
pub(crate) struct CachingDeleteFileLoader {
    basic_delete_file_loader: BasicDeleteFileLoader,
    #[allow(dead_code)]
    concurrency_limit_data_files: usize,
    /// Shared filter state with OnceLock-based caching.
...
impl CachingDeleteFileLoader {
    ...
    ///
    /// For each delete file, the OnceLock mechanism ensures:
    /// - If already loaded, returns immediately
    /// - If being loaded by another thread, blocks and waits
    /// - If not loaded, this thread performs the load
    ///
    /// Returns the DeleteFilter with all delete data loaded and cached.
    pub(crate) fn load_deletes(
        ...
        Ok(self.delete_filter.clone())
    }

    /// Loads a positional delete file.
    ///
    ...
    ///
    /// Returns a predicate that filters out rows matching the deleted values.
    fn parse_equality_deletes_record_batch_iterator(
        iterator: ArrowRecordBatchIterator,
...
mod tests {
    ...
    #[test]
    fn test_partial_schema_equality_deletes_evolve_succeeds() {
        let tmp_dir = TempDir::new().unwrap();
        let table_location = tmp_dir.path().as_os_str().to_str().unwrap();
    ...
    #[test]
    fn test_load_deletes_with_mixed_types() {
        ...
        let result = delete_filter.build_equality_delete_predicate(&file_scan_task);
        assert!(
            result.is_ok(),
    ...
    #[test]
    fn test_large_equality_delete_batch_stack_overflow() {
        let tmp_dir = TempDir::new().unwrap();
        let table_location = tmp_dir.path().as_os_str().to_str().unwrap();
        let file_io = FileIO::from_path(table_location).unwrap();
