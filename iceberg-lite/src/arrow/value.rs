...
use arrow_array::{
    ...
    Int64Array, LargeBinaryArray, LargeListArray, LargeStringArray, ListArray,
    MapArray, StringArray, StructArray, Time64MicrosecondArray,
...
use crate::spec::{
    ...
    PrimitiveType, SchemaWithPartnerVisitor, Struct, StructType, Type, VariantType,
...
impl SchemaWithPartnerVisitor<ArrayRef> for ArrowArrayToIcebergStructConverter {
    ...
    fn r#struct(
        ...
        for i in 0..row_len {
            ...
            if array.is_null(i) {
                ...
                struct_literals
                    .push(Some(Literal::Struct(Struct::from_iter(literals))));
            }
        }
            ...
            DataType::List(_) => {
                let offset = array
                    ...
                    .ok_or_else(|| {
                        Error::new(
                            ErrorKind::DataInvalid,
                            "The partner is not a list array",
                        )
                    })?
                    .offsets();
    ...
    fn map(
        ...
        let offsets = partner
            ...
            .ok_or_else(|| {
                Error::new(ErrorKind::DataInvalid, "The partner is not a map array")
            })?
            .offsets();
        // combine the result according to the offset
        ...
        for i in 0..offsets.len() - 1 {
            ...
            let mut map = Map::new();
            for (key, value) in
                key_values[start..end].iter().zip(values[start..end].iter())
            {
    ...
    fn primitive(
        ...
        match p {
            PrimitiveType::Boolean => {
                let array = partner
                    ...
                    .ok_or_else(|| {
                        Error::new(
                            ErrorKind::DataInvalid,
                            "The partner is not a boolean array",
                        )
                    })?;
            ...
            PrimitiveType::Int => {
                let array = partner
                    ...
                    .ok_or_else(|| {
                        Error::new(
                            ErrorKind::DataInvalid,
                            "The partner is not a int32 array",
                        )
                    })?;
            ...
            PrimitiveType::Long => {
                let array = partner
                    ...
                    .ok_or_else(|| {
                        Error::new(
                            ErrorKind::DataInvalid,
                            "The partner is not a int64 array",
                        )
                    })?;
            ...
            PrimitiveType::Float => {
                let array = partner
                    ...
                    .ok_or_else(|| {
                        Error::new(
                            ErrorKind::DataInvalid,
                            "The partner is not a float32 array",
                        )
                    })?;
            ...
            PrimitiveType::Double => {
                let array = partner
                    ...
                    .ok_or_else(|| {
                        Error::new(
                            ErrorKind::DataInvalid,
                            "The partner is not a float64 array",
                        )
                    })?;
            ...
            PrimitiveType::Date => {
                let array = partner
                    ...
                    .ok_or_else(|| {
                        Error::new(
                            ErrorKind::DataInvalid,
                            "The partner is not a date32 array",
                        )
                    })?;
            ...
            PrimitiveType::Time => {
                let array = partner
                    ...
                    .ok_or_else(|| {
                        Error::new(
                            ErrorKind::DataInvalid,
                            "The partner is not a time64 array",
                        )
                    })?;
            ...
            PrimitiveType::String => {
                if let Some(array) =
                    partner.as_any().downcast_ref::<LargeStringArray>()
                {
            ...
            PrimitiveType::Uuid => {
                if let Some(array) =
                    partner.as_any().downcast_ref::<FixedSizeBinaryArray>()
                {
            ...
            PrimitiveType::Fixed(len) => {
                let array = partner
                    ...
                    Error::new(
                        ErrorKind::DataInvalid,
                        "The partner is not a fixed array",
                    )
                })?;
            ...
            PrimitiveType::Binary => {
                if let Some(array) =
                    partner.as_any().downcast_ref::<LargeBinaryArray>()
                {
                    Ok(array
                        .iter()
                    ...
                    partner.as_any().downcast_ref::<BinaryArray>()
                {
                    Ok(array
                        .iter()
                        .map(|v| v.map(|v| Literal::binary(v.to_vec())))
        ...
        let map_array =
            map_partner
                ...
                .ok_or_else(|| {
                    Error::new(
                        ErrorKind::DataInvalid,
                        "The map partner is not a map array",
                    )
                })?;
        Ok(map_array.keys())
    }
...
pub(crate) fn create_primitive_array_single_element(
    ...
    match (data_type, prim_lit) {
        ...
        _ => Err(Error::new(
            ErrorKind::Unexpected,
            format!(
                "Unsupported constant type combination: {data_type:?} with {prim_lit:?}"
            ),
        )),
    }
}
...
mod test {
    ...
    use arrow_array::builder::{
        Int32Builder, ListBuilder, MapBuilder, StructBuilder,
    };
    use arrow_array::{
    ...
    use super::*;
    use crate::spec::{
        ListType, Literal, MapType, NestedField, PrimitiveType, StructType, Type,
    };
    ...
    fn test_arrow_struct_to_iceberg_struct() {
        ...
        let time_array = Time64MicrosecondArray::from(vec![
            Some(123456789),
            Some(987654321),
            None,
        ]);
        ...
        let result =
            arrow_struct_to_literal(&struct_array, &iceberg_struct_type).unwrap();
        ...
        let struct_array = {
            let mut builder = StructBuilder::from_fields(
                Fields::from(vec![
                    ...
                    Field::new("b", DataType::Int32, true).with_metadata(
                        HashMap::from([(
                            PARQUET_FIELD_ID_META_KEY.to_string(),
                            "1".to_string(),
                        )]),
    ...
    fn test_empty_struct() {
        let struct_array =
            Arc::new(StructArray::new_null(Fields::empty(), 3)) as ArrayRef;
        let iceberg_struct_type = StructType::new(vec![]);
        let result =
            arrow_struct_to_literal(&struct_array, &iceberg_struct_type).unwrap();
    ...
    fn test_arrow_variant_to_literal_is_unsupported() {
        ...
        let struct_array = Arc::new(StructArray::from(vec![(
            Arc::new(
                Field::new("v", variant_child.data_type().clone(), false)
                    .with_metadata(HashMap::from([(
                        PARQUET_FIELD_ID_META_KEY.to_string(),
                        "1".to_string(),
                    )])),
    ...
    fn test_find_field_by_id() {
        ...
        let field_b_array =
            StringArray::from(vec![Some("value1"), Some("value2"), None]);
        ...
        let field_a_partner =
            accessor.field_partner(nested_partner, &field_a).unwrap();
    ...
    fn test_complex_nested() {
        ...
        let struct_array = {
            ...
            let a_struct_builder = StructBuilder::new(
                vec![
                    Field::new("a1", DataType::Int32, false).with_metadata(
                        HashMap::from([(
                            PARQUET_FIELD_ID_META_KEY.to_string(),
                            "2".to_string(),
                        )]),
                    ),
                    Field::new("a2", DataType::Int32, false).with_metadata(
                        HashMap::from([(
                            PARQUET_FIELD_ID_META_KEY.to_string(),
                            "3".to_string(),
                        )]),
            ...
            let map_builder =
                MapBuilder::new(None, map_key_builder, map_value_builder);
            ...
            let mut top_struct_builder = {
                let a_struct_type = DataType::Struct(Fields::from(vec![
                    Field::new("a1", DataType::Int32, false).with_metadata(
                        HashMap::from([(
                            PARQUET_FIELD_ID_META_KEY.to_string(),
                            "2".to_string(),
                ...
                let a_type = DataType::List(Arc::new(Field::new(
                    "item",
                    a_struct_type.clone(),
                    true,
                )));
                ...
                let b_map_type = DataType::Map(
                    Arc::new(b_map_entry_struct),
                    /* sorted_keys = */ false,
                );
                let b_type = DataType::List(Arc::new(Field::new(
                    "item",
                    b_map_type.clone(),
                    true,
                )));

                let c_inner_list_type = DataType::List(Arc::new(Field::new(
                    "item",
                    DataType::Int32,
                    true,
                )));
                ...
                StructBuilder::new(
                    Fields::from(vec![
                        Field::new("A", a_type.clone(), false).with_metadata(
                            HashMap::from([(
                                PARQUET_FIELD_ID_META_KEY.to_string(),
                                "0".to_string(),
                            )]),
                        ),
                        Field::new("B", b_type.clone(), false).with_metadata(
                            HashMap::from([(
                                PARQUET_FIELD_ID_META_KEY.to_string(),
                                "4".to_string(),
                            )]),
                        ),
                        Field::new("C", c_type.clone(), false).with_metadata(
                            HashMap::from([(
                                PARQUET_FIELD_ID_META_KEY.to_string(),
                                "8".to_string(),
                            )]),
