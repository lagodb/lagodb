...
use crate::spec::{
    ...
    ManifestEntry, ManifestFile, ManifestListWriter, ManifestWriter,
...
impl<'a> SnapshotProducer<'a> {
    ...
    fn write_added_manifest(&mut self) -> Result<ManifestFile> {
    ...
    fn manifest_file<OP: SnapshotProduceOperation, MP: ManifestProcess>(
        &mut self,
        ...
        let existing_manifests =
            snapshot_produce_operation.existing_manifest(self)?;
        let mut manifest_files = existing_manifests;

        // Process added entries.
        if !self.added_data_files.is_empty() {
            let added_manifest = self.write_added_manifest()?;
    ...
    fn summary<OP: SnapshotProduceOperation>(
        ...
        // Computed metrics must win over user properties, matching Iceberg Java's
        // SnapshotSummary.Builder ordering.
    ...
    pub(crate) fn commit<OP: SnapshotProduceOperation, MP: ManifestProcess>(
        mut self,
        snapshot_produce_operation: OP,
        process: MP,
    ) -> Result<ActionCommit> {
        ...
        let new_manifests =
            self.manifest_file(&snapshot_produce_operation, &process)?;

        manifest_list_writer.add_manifests(new_manifests.into_iter())?;
        let writer_next_row_id = manifest_list_writer.next_row_id();
        manifest_list_writer.close()?;
        ...
        let new_snapshot = if let Some(writer_next_row_id) = writer_next_row_id {
            let assigned_rows = writer_next_row_id
                .checked_sub(self.table.metadata().next_row_id())
                .ok_or_else(|| {
                    Error::new(
                        ErrorKind::DataInvalid,
                        "snapshot row-id assignment moved backwards",
                    )
                })?;
            new_snapshot
