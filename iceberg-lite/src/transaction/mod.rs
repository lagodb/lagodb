            ...
            match self.do_commit(catalog) {
                ...
                Err(e) if e.retryable() => {
                    ...
                    match backoff.next() {
                        ...
                        None => {
                            // Backoff exhausted, return the error
                            return Err(e);
                        }
                    }
                }
...
mod tests {
    ...
    pub(crate) fn make_v3_minimal_table_in_catalog(catalog: &impl Catalog) -> Table {
        let table_ident = TableIdent::from_strs([
            format!("ns1-{}", uuid::Uuid::new_v4()),
            "test1".to_string(),
        ])
        .unwrap();
        ...
        let reader = BufReader::new(file);
        let base_metadata =
            serde_json::from_reader::<_, TableMetadata>(reader).unwrap();
    ...
    #[test]
    fn test_commit_retryable_error() {
        ...
        let result = tx.commit(&mock_catalog);
    ...
    #[test]
    fn test_commit_non_retryable_error() {
        // Create a test table with retry properties
        let table = setup_test_table("3");
    ...
    #[test]
    fn test_commit_max_retries_exceeded() {
...
mod test_row_lineage {
    ...
    #[test]
    fn test_fast_append_with_row_lineage() {
