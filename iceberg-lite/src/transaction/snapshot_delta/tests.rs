...
// under the License.
    ...
    use crate::io::{
        FileIO, FileMetadata, FileWrite, OpenedFile, Storage,
    };
    ...
    fn rewrite_manifests_streams_entries_into_fewer_manifests() {
        ...
        for index in 0..6 {
            ...
            let tx = tx
                .fast_append()
                .add_data_files([data_file(&format!(
                    "test/manifest-{index}.parquet"
                ))])
                .apply(tx)
                .unwrap();
    ...
    fn expire_snapshots_preserves_retained_refs_then_removes_aged_refs() {
        ...
        let retained_metadata = table
            ...
            .metadata;
        let retained_table =
            table.clone().with_metadata(Arc::new(retained_metadata));
