...
use crate::spec::{
    NestedField, NestedFieldRef, PartitionField, PartitionSpec, Schema, StructType,
    Transform, Type,
};
...
mod tests {
    ...
    use super::*;
    use crate::spec::{
        NestedField, PrimitiveType, Transform, Type, UnboundPartitionSpec,
    };
    ...
    fn test_single_spec_identity() {
        ...
        let result =
            compute_unified_partition_type([&spec].into_iter(), &schema).unwrap();
    ...
    fn test_newer_name_takes_precedence() {
        ...
        let result =
            compute_unified_partition_type([&spec_v0, &spec_v1].into_iter(), &schema)
                .unwrap();
        assert_eq!(result.fields().len(), 1);
    ...
    fn test_evolution_adds_new_field() {
        ...
        let spec_v0 =
            build_spec(&schema, 0, vec![(4, "category", Transform::Identity)]);
    ...
    fn test_unknown_transform_errors() {
        ...
        let err =
            compute_unified_partition_type([&spec].into_iter(), &schema).unwrap_err();
