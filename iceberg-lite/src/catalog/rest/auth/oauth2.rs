...
use super::super::catalog::{
    ...
    default_token_endpoint, explicit_headers_from_props,
};
use super::super::client::HttpClient;
use super::super::types::{ErrorResponse, OAuthError, TokenResponse};
...
impl OAuth2Manager {
    ...
    pub fn new(token_endpoint: impl Into<String>) -> Self {
        Self {
            ...
            init_params: OAuth2Params {
                ...
                extra_oauth_params: HashMap::from([(
                    "scope".to_string(),
                    "catalog".to_string(),
                )]),
            },
    ...
    /// `catalog` scope.
    pub fn with_extra_oauth_params(
        mut self,
...
impl Debug for OAuth2Manager {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OAuth2Manager").finish_non_exhaustive()
    }
}
...
impl AuthManager for OAuth2Manager {
    fn init_session(
        ...
        Ok(Box::new(self.session_from(client, props)?))
    }
    ...
    fn catalog_session(
        ...
        Ok(Arc::new(self.session_from(client, props)?))
    }
}
...
impl OAuth2Manager {
    ...
    fn session_from(
        ...
        if let Some(token) = props.get("token") {
            *self.token.lock() = Some(SensitiveString::from(token.clone()));
        }
        ...
        Ok(OAuth2Session {
            ...
            token_source: match credential {
                Some(credential) => TokenSource::ClientCredentials(Box::new(
...
impl Debug for OAuth2Session {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OAuth2Session").finish_non_exhaustive()
    }
}

impl ClientCredentialsConfig {
    fn exchange_credential_for_token(&self) -> Result<String> {
        ...
        let (status, body) = self.client.post_form(
            &self.token_endpoint,
            &self.extra_headers,
            &params,
        )?;
        ...
        if status != StatusCode::OK {
            ...
            let error =
                serde_json::from_slice::<OAuthError>(&body).map_err(|error| {
...
impl AuthSession for OAuth2Session {
    ...
    fn authenticate(&self, req: &mut HttpRequest) -> Result<()> {
        ...
        let token = {
            ...
            match (&*token, &self.token_source) {
                ...
                (None, TokenSource::ClientCredentials(config)) => {
                    let new_token = SensitiveString::from(
                        config.exchange_credential_for_token()?,
                    );
