...
pub use super::table_metadata_builder::{
    TableMetadataBuildResult, TableMetadataBuilder,
};
use super::{
    DEFAULT_PARTITION_SPEC_ID, PartitionSpecRef, PartitionStatisticsFile, SchemaId,
...
impl TableMetadata {
    ...
    pub fn partition_specs_iter(
        &self,
    ) -> impl ExactSizeIterator<Item = &PartitionSpecRef> {
    ...
    pub fn current_snapshot(&self) -> Option<&SnapshotRef> {
        self.current_snapshot_id.map(|s| {
            self.snapshot_by_id(s).expect(
                "Current snapshot id has been set, but doesn't exist in metadata",
            )
        })
    }
    ...
    pub fn snapshot_for_ref(&self, ref_name: &str) -> Option<&SnapshotRef> {
        self.refs.get(ref_name).map(|r| {
            self.snapshot_by_id(r.snapshot_id).unwrap_or_else(|| {
                panic!("Snapshot id of ref {ref_name} doesn't exist")
            })
        })
    }
    ...
    #[inline]
    pub fn encryption_keys_iter(
        &self,
    ) -> impl ExactSizeIterator<Item = &EncryptedKey> {
    ...
    pub fn read_from(
        ...
        let metadata_content = input_file.read()?;
    ...
    pub fn write_to(
        ...
        if let Ok(parsed_location) = MetadataLocation::from_str(metadata_location)
            && codec != parsed_location.compression_codec()
        {
            return Err(Error::new(
                ErrorKind::DataInvalid,
                format!(
                    "Compression codec mismatch: metadata_location has {:?}, but table properties specify {:?}",
                    parsed_location.compression_codec(),
                    codec
                ),
            ));
        }
        ...
        let data_to_write = match codec {
            CompressionCodec::Gzip(_) => codec.compress(json_bytes)?,
            CompressionCodec::None => json_bytes,
            _ => {
                return Err(Error::new(
                    ErrorKind::DataInvalid,
        ...
        file_io.new_output(metadata_location)?.write(&data_to_write)
    }
    ...
    fn validate_snapshot_sequence_number(&self) -> Result<()> {
        ...
        if self.format_version >= FormatVersion::V2
            && let Some(snapshot) = self.snapshots.values().find(|snapshot| {
...
pub(super) mod _serde {
    ...
    use super::{
        DEFAULT_PARTITION_SPEC_ID, EMPTY_SNAPSHOT_ID, FormatVersion, MAIN_BRANCH,
        MetadataLog, SnapshotLog, TableMetadata,
    };
    ...
    use crate::spec::{
        ...
        PartitionSpecRef, PartitionStatisticsFile, Schema, SchemaRef, Snapshot,
        SnapshotReference, SnapshotRetention, SortOrder, StatisticsFile,
    };
    use crate::{Error, ErrorKind};
    ...
    impl TryFrom<TableMetadataV3> for TableMetadata {
        ...
        fn try_from(value: TableMetadataV3) -> Result<Self, self::Error> {
            ...
            let current_snapshot_id =
                if value.current_snapshot_id == Some(EMPTY_SNAPSHOT_ID) {
                    None
                } else {
            ...
            let mut metadata = TableMetadata {
                ...
                partition_statistics: index_partition_statistics(
                    value.partition_statistics,
                ),
                encryption_keys: encryption_keys
                    .map(|keys| {
                        HashMap::from_iter(
                            keys.into_iter().map(|key| (key.key_id.clone(), key)),
                        )
                    })
                    .unwrap_or_default(),
                next_row_id,
    ...
    impl TryFrom<TableMetadataV1> for TableMetadata {
        ...
        fn try_from(value: TableMetadataV1) -> Result<Self, Error> {
            ...
            let (schemas, current_schema_id, current_schema) = if let (
                Some(schemas_vec),
                Some(schema_id),
            ) =
                (&value.schemas, value.current_schema_id)
            {
                ...
                let schema_map =
                    HashMap::from_iter(vec![(schema_id, schema_arc.clone())]);
            ...
            let partition_specs = if let Some(specs_vec) = value.partition_specs {
                ...
                let spec = PartitionSpec::builder(current_schema.clone())
                    ...
                    .add_unbound_fields(
                        partition_spec.into_iter().map(|f| f.into_unbound()),
                    )?
                    .build()?;
            ...
            let default_spec_id = value.default_spec_id.unwrap_or_else(|| {
        ...
        fn from(v: TableMetadata) -> Self {
            TableMetadataV2V3Shared {
                ...
                partition_specs: v
                    ...
                    .into_values()
                    .map(|x| {
                        Arc::try_unwrap(x).unwrap_or_else(|s| s.as_ref().clone())
                    })
                    .collect(),
            ...
            Ok(TableMetadataV1 {
                ...
                sort_orders: Some(
                    v.sort_orders
                        .into_values()
                        .map(|s| {
                            Arc::try_unwrap(s).unwrap_or_else(|s| s.as_ref().clone())
                        })
                        .collect(),
                ),
...
#[derive(
    Debug, Serialize_repr, Deserialize_repr, PartialEq, Eq, Clone, Copy, Hash,
)]
#[repr(u8)]
...
mod tests {
    ...
    use crate::spec::{
        BlobMetadata, EncryptedKey, INITIAL_ROW_ID, Literal, NestedField, NullOrder,
        Operation, PartitionSpec, PartitionStatisticsFile, PrimitiveLiteral,
        PrimitiveType, Schema, Snapshot, SnapshotReference, SnapshotRetention,
        SortDirection, SortField, SortOrder, StatisticsFile, Summary, Transform,
    ...
    fn test_table_data_v2() {
        ...
        let expected = TableMetadata {
            format_version: FormatVersion::V2,
            table_uuid: Uuid::parse_str("fb072c92-a02b-11e9-ae9c-1bb7bc9eca94")
                .unwrap(),
    ...
    fn test_table_data_v3() {
        ...
        let expected = TableMetadata {
            ...
            statistics: HashMap::new(),
            partition_statistics: HashMap::new(),
            encryption_keys: HashMap::from_iter(vec![(
                "key1".to_string(),
                encryption_key,
            )]),
            next_row_id: 5,
    ...
    "#;
        ...
        let expected = TableMetadata {
            format_version: FormatVersion::V2,
            table_uuid: Uuid::parse_str("9c12d441-03fe-4693-9a96-a0705ddf69c1")
                .unwrap(),
    ...
    fn test_table_metadata_v3_valid_minimal() {
        let metadata_str = fs::read_to_string(
        ...
        let schema = Schema::builder()
            ...
            .with_fields(vec![
                Arc::new(
                    ...
                    .with_initial_default(Literal::Primitive(PrimitiveLiteral::Long(
                        1,
                    )))
    ...
    fn test_table_metadata_v2_file_valid() {
        let metadata =
            fs::read_to_string("testdata/table_metadata/TableMetadataV2Valid.json")
                .unwrap();
    ...
    fn test_table_metadata_v1_file_valid() {
        ...
        let expected = TableMetadata {
            format_version: FormatVersion::V1,
            table_uuid: Uuid::parse_str("d20125c8-7284-442c-9aea-15fee620737c")
                .unwrap(),
            location: "s3://bucket/test/location".to_string(),
    ...
    fn test_table_metadata_v1_compat() {
        let metadata =
            fs::read_to_string("testdata/table_metadata/TableMetadataV1Compat.json")
                .unwrap();
    ...
    fn test_table_metadata_v1_schemas_without_current_id() {
        ...
        let desered_type: TableMetadata = serde_json::from_str(&metadata).expect(
            "Failed to deserialize TableMetadataV1SchemasWithoutCurrentId.json",
        );
    ...
    fn test_table_metadata_v1_no_valid_schema() {
        let metadata = fs::read_to_string(
            "testdata/table_metadata/TableMetadataV1NoValidSchema.json",
        )
        .unwrap();
    ...
    fn test_table_metadata_v2_missing_sort_order() {
        let metadata = fs::read_to_string(
            "testdata/table_metadata/TableMetadataV2MissingSortOrder.json",
        )
        .unwrap();
    ...
    fn test_table_metadata_v2_missing_partition_specs() {
        let metadata = fs::read_to_string(
            "testdata/table_metadata/TableMetadataV2MissingPartitionSpecs.json",
        )
        .unwrap();
    ...
    fn test_table_metadata_v2_missing_schemas() {
        let metadata = fs::read_to_string(
            "testdata/table_metadata/TableMetadataV2MissingSchemas.json",
        )
        .unwrap();
    ...
    fn test_table_metadata_v2_unsupported_version() {
        let metadata = fs::read_to_string(
            "testdata/table_metadata/TableMetadataUnsupportedVersion.json",
        )
        .unwrap();
            ...
            HashMap::from([(
                ...
                Arc::new(
                    PartitionSpec::builder(
                        table_metadata.schemas.get(&0).unwrap().clone()
                    )
                    .with_spec_id(0)
                    .build()
                    .unwrap()
    ...
    #[test]
    fn test_table_metadata_read_write() {
        ...
        let read_metadata =
            TableMetadata::read_from(&file_io, &metadata_location).unwrap();
    ...
    #[test]
    fn test_table_metadata_read_compressed() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_table_metadata_read_nonexistent_file() {
        // Create a FileIO instance
        let file_io = FileIO::local();

        // Try to read a non-existent file
        let result =
            TableMetadata::read_from(&file_io, "/nonexistent/path/metadata.json");
    ...
    fn test_v2_to_v3_upgrade_preserves_existing_snapshots_without_row_lineage() {
        ...
        let v3_json = serde_json::to_string(&v3_metadata).unwrap();
        let deserialized: TableMetadata = serde_json::from_str(&v3_json).unwrap();
    ...
    fn test_v3_snapshot_with_row_lineage_serialization() {
        ...
        let v3_json = serde_json::to_string(&v3_with_snapshot).unwrap();
        let deserialized: TableMetadata = serde_json::from_str(&v3_json).unwrap();

        assert_eq!(deserialized.format_version, FormatVersion::V3);
        assert_eq!(deserialized.snapshots.len(), 1);
        assert_eq!(
            deserialized.snapshots.values().next().unwrap().row_range(),
            Some((100, 50))
        );
    }
