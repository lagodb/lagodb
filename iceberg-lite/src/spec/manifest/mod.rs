...
use super::{
    Datum, FormatVersion, ManifestContentType, PartitionSpec, PrimitiveType, Schema,
    Struct, UNASSIGNED_SEQUENCE_NUMBER,
};
...
pub fn serialize_data_file_to_json(
    ...
    let serde =
        _serde::DataFileSerde::try_from(data_file, partition_type, format_version)?;
...
mod tests {
    ...
    use crate::{
        io::FileIO,
        spec::{Literal, NestedField, PrimitiveType, Struct, Transform, Type},
    };
    ...
    fn test_parse_manifest_v2_partition() {
        ...
        let manifest_file = writer.write_manifest_file().unwrap();
    ...
    #[test]
    fn test_parse_manifest_with_schema_evolution() {
        let schema = Arc::new(
            Schema::builder()
                .with_fields(vec![
                    Arc::new(NestedField::optional(
    ...
    fn test_manifest_summary() {
        ...
        let res = writer.write_manifest_file().unwrap();
    ...
    fn test_data_file_serialization() {
        ...
        let serialized_files = data_files
            ...
            .into_iter()
            .map(|f| {
                serialize_data_file_to_json(f, &partition_type, FormatVersion::V2)
                    .unwrap()
            })
            .collect::<Vec<String>>();
        ...
        let pretty_json1: Value =
            serde_json::from_str(serialized_files.first().unwrap()).unwrap();
