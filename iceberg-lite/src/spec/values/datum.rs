    ...
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match (&self.literal, &other.literal, &self.r#type, &other.r#type) {
            ...
            (
                ...
                let other_val =
                    decimal_from_i128_with_scale(*other_val, *other_scale);
                val.partial_cmp(&other_val)
            }
            _ => None,
        }
...
impl Datum {
    ...
    pub fn try_from_bytes(bytes: &[u8], data_type: PrimitiveType) -> Result<Self> {
        let literal = match data_type {
            ...
            PrimitiveType::Decimal { .. } => PrimitiveLiteral::Int128(
                i128_from_be_bytes(bytes).ok_or_else(|| {
                    Error::new(
    ...
    pub fn to_bytes(&self) -> Result<ByteBuf> {
        let buf = match &self.literal {
            ...
            PrimitiveLiteral::Int128(val) => {
                ...
                // The primitive literal is the unscaled value. Convert into
                // two's-complement byte representation in big-endian byte order.
    ...
    /// Example:
    /// ```rust
    /// use iceberg_lite::spec::{Datum, Literal, PrimitiveLiteral};
    ...
    /// Example
    /// ```rust
    /// use iceberg_lite::spec::{Datum, Literal};
