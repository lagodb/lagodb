...
use arrow_array::builder::{
    BooleanBuilder, GenericListBuilder, ListBuilder, PrimitiveBuilder, StringBuilder,
    StructBuilder,
};
...
impl<'a> ManifestsTable<'a> {
    ...
    pub fn scan(&self) -> Result<ArrowRecordBatchIterator> {
        ...
        Ok(Box::new(vec![Ok(batch)].into_iter()))
    }

    fn partition_summary_builder(
        &self,
...
mod tests {
    ...
    #[test]
    fn test_manifests_table() {
        let mut fixture = TableTestFixture::new();
        fixture.setup_manifest_files();

        let record_batch = fixture.table.inspect().manifests().scan().unwrap();

        check_record_batches(
            record_batch.collect::<Result<Vec<_>>>().unwrap(),
            expect![[r#"
