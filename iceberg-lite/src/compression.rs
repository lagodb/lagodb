    ...
    pub(crate) fn compress(&self, bytes: Vec<u8>) -> Result<Vec<u8>> {
        match self {
            ...
            CompressionCodec::Gzip(level) => {
                let compression =
                    Compression::new((*level).min(GZIP_MAX_LEVEL) as u32);
...
mod tests {
    ...
    #[test]
    fn test_compression_codec_none() {
        let bytes = [0_u8; 100].to_vec();

        let codec = CompressionCodec::None;
        let compressed = codec.compress(bytes.clone()).unwrap();
    ...
    #[test]
    fn test_compression_codec_compress() {
        let bytes = [0_u8; 100].to_vec();
    ...
    #[test]
    fn test_compression_codec_unsupported() {
        let bytes = [0_u8; 100].to_vec();

        for codec in [CompressionCodec::Lz4, CompressionCodec::Snappy] {
            assert!(codec.compress(bytes.clone()).is_err());
            assert!(codec.decompress(bytes.clone()).is_err());
        }
    }
}
