...
impl FileMetadata {
    ...
    pub(crate) const MAGIC: [u8; FileMetadata::MAGIC_LENGTH as usize] =
        [0x50, 0x46, 0x41, 0x31];
    ...
    const FOOTER_STRUCT_MAGIC_OFFSET: u8 = FileMetadata::FOOTER_STRUCT_FLAGS_OFFSET
    ...
    fn read_footer_payload_length(
        ...
        let footer_payload_length_bytes = file_read.read_range(start..end)?;
        let mut buf = [0; 4];
    ...
    fn extract_footer_payload_as_str(
        ...
        let footer_compression_codec =
            if flags.contains(&Flag::FooterPayloadCompressed) {
        ...
        let end_offset = FileMetadata::MAGIC_LENGTH as usize
            + usize::try_from(footer_payload_length)?;
        let footer_payload_bytes =
            footer_bytes.get(start_offset..end_offset).ok_or_else(|| {
                Error::new(ErrorKind::DataInvalid, "Index range is out of bounds.")
            })?;
    ...
    fn read_from(
        ...
        FileMetadata::check_magic(
            &footer_bytes[footer_bytes.len() - magic_length..],
        )?;

        let footer_payload_str = FileMetadata::extract_footer_payload_as_str(
            &footer_bytes,
            footer_payload_length,
        )?;
    ...
    pub(crate) fn read_with_prefetch(
        ...
        if prefetch_hint > 16 {
            ...
            let payload_length_end = payload_length_start
...
mod tests {
    ...
    fn input_file_with_bytes(temp_dir: &TempDir, slice: &[u8]) -> InputFile {
        ...
        output_file.write(slice).unwrap();

        output_file.to_input_file()
    }

    fn input_file_with_payload(temp_dir: &TempDir, payload_str: &str) -> InputFile {
        ...
        input_file_with_bytes(temp_dir, &bytes)
    }

    #[test]
    fn test_file_starting_with_invalid_magic_returns_error() {
        ...
        assert_eq!(
            FileMetadata::read(&input_file).unwrap_err().to_string(),
            "DataInvalid => Bad magic value: [80, 70, 65, 0] should be [80, 70, 65, 49]",
        )
    }

    #[test]
    fn test_file_with_invalid_magic_at_start_of_footer_returns_error() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_file_ending_with_invalid_magic_returns_error() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_encoded_payload_length_larger_than_actual_payload_length_returns_error() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_encoded_payload_length_smaller_than_actual_payload_length_returns_error()
    {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_lz4_compressed_footer_returns_error() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_unknown_byte_bit_combination_returns_error() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_non_utf8_string_payload_returns_error() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_minimal_valid_file_returns_file_metadata() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_returns_file_metadata_property() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_returns_file_metadata_properties() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_returns_error_if_blobs_field_is_missing() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_returns_error_if_blobs_field_is_bad() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_returns_blobs_metadatas() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_returns_properties_in_blob_metadata() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_returns_error_if_blobs_fields_value_is_outside_i32_range() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_returns_errors_if_footer_payload_is_not_encoded_in_json_format() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_read_file_metadata_of_uncompressed_empty_file() {
        let input_file = java_empty_uncompressed_input_file();

        let file_metadata = FileMetadata::read(&input_file).unwrap();
        assert_eq!(file_metadata, empty_footer_payload())
    }

    #[test]
    fn test_read_file_metadata_of_uncompressed_metric_data() {
    ...
    #[test]
    fn test_read_file_metadata_of_zstd_compressed_metric_data() {
    ...
    #[test]
    fn test_read_file_metadata_of_empty_file_with_prefetching() {
        let input_file = java_empty_uncompressed_input_file();
        let file_metadata =
            FileMetadata::read_with_prefetch(&input_file, 64).unwrap();
    ...
    #[test]
    fn test_read_file_metadata_of_uncompressed_metric_data_with_prefetching() {
        let input_file = java_uncompressed_metric_input_file();
        let file_metadata =
            FileMetadata::read_with_prefetch(&input_file, 64).unwrap();
    ...
    #[test]
    fn test_read_file_metadata_of_zstd_compressed_metric_data_with_prefetching() {
        let input_file = java_zstd_compressed_metric_input_file();
        let file_metadata =
            FileMetadata::read_with_prefetch(&input_file, 64).unwrap();
