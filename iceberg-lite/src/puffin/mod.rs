...
mod tests {
    ...
    fn test_puffin_codec_validation() {
        ...
        assert!(
            validate_puffin_compression(CompressionCodec::zstd_default()).is_ok()
        );
        assert!(validate_puffin_compression(CompressionCodec::Zstd(5)).is_ok());

        assert!(
            validate_puffin_compression(CompressionCodec::gzip_default()).is_err()
        );
