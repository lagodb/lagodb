    ...
    F: FileNameGenerator,
{
    ...
    pub fn write(
            ...
            && let Some(inner) = self.inner.take()
        {
            ...
            self.data_file_builders.extend(inner.close()?);
    ...
    pub fn close(mut self) -> Result<Vec<DataFileBuilder>> {
        // close the current writer and merge the output
        if let Some(current_writer) = self.inner {
            self.data_file_builders.extend(current_writer.close()?);
        }
...
mod tests {
    ...
    #[test]
    fn test_rolling_writer_basic() -> Result<()> {
        ...
        // Verify file content
        check_parquet_data_file(&file_io, &data_files[0], &batch);

        Ok(())
    }

    #[test]
    fn test_rolling_writer_with_rolling() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let file_io = FileIO::local();
        let location_gen = DefaultLocationGenerator::with_data_location(
            temp_dir.path().to_str().unwrap().to_string(),
