    ...
    <O as IntoIterator>::Item: Send + Clone,
{
    ...
    fn close_current_writer(&mut self) -> Result<()> {
        if let Some(mut writer) = self.current_writer.take() {
            self.output.extend(writer.close()?);
...
where
    ...
    <O as IntoIterator>::Item: Send + Clone,
{
    fn write(&mut self, partition_key: PartitionKey, input: I) -> Result<()> {
        ...
        if need_new_writer {
            ...
            self.current_writer =
                Some(self.inner_builder.build(Some(partition_key.clone()))?);
...
mod tests {
    ...
    #[test]
    fn test_clustered_writer_single_partition() -> Result<()> {
        ...
        let partition_spec =
            crate::spec::PartitionSpec::builder(schema.clone()).build()?;
        let partition_value = crate::spec::Struct::from_iter([Some(
            crate::spec::Literal::string("US"),
        )]);
        ...
        writer.write(partition_key.clone(), batch2)?;
    ...
    #[test]
    fn test_clustered_writer_sorted_partitions() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let file_io = FileIO::local();
        let location_gen = DefaultLocationGenerator::with_data_location(
            temp_dir.path().to_str().unwrap().to_string(),
        ...
        let partition_value_asia = crate::spec::Struct::from_iter([Some(
            crate::spec::Literal::string("ASIA"),
        )]);
        ...
        let partition_value_eu = crate::spec::Struct::from_iter([Some(
            crate::spec::Literal::string("EU"),
        )]);
        ...
        let partition_value_us = crate::spec::Struct::from_iter([Some(
            crate::spec::Literal::string("US"),
        )]);
        ...
        writer.write(partition_key_asia.clone(), batch_asia)?;
        writer.write(partition_key_eu.clone(), batch_eu)?;
        writer.write(partition_key_us.clone(), batch_us)?;
    ...
    #[test]
    fn test_clustered_writer_unsorted_partitions_error() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let file_io = FileIO::local();
        let location_gen = DefaultLocationGenerator::with_data_location(
            temp_dir.path().to_str().unwrap().to_string(),
        ...
        let result = writer.write(partition_key_us.clone(), batch_us2);
