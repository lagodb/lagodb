    ...
    <O as IntoIterator>::Item: Send + Clone,
{
    fn write(&mut self, partition_key: PartitionKey, input: I) -> Result<()> {
        let writer = self.get_or_create_writer(&partition_key)?;
        writer.write(input)
    }
...
mod tests {
    ...
    #[test]
    fn test_fanout_writer_single_partition() -> Result<()> {
    ...
    #[test]
    fn test_fanout_writer_multiple_partitions() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let file_io = FileIO::local();
        let location_gen = DefaultLocationGenerator::with_data_location(
            temp_dir.path().to_str().unwrap().to_string(),
        ...
        writer.write(partition_key_us.clone(), batch_us1)?;
        writer.write(partition_key_eu.clone(), batch_eu1)?;
        writer.write(partition_key_us.clone(), batch_us2)?; // Back to US partition
        writer.write(partition_key_asia.clone(), batch_asia1)?;
