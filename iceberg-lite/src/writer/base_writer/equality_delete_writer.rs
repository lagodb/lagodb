...
mod test {
    ...
    #[test]
    fn test_equality_delete_writer() -> Result<(), anyhow::Error> {
    ...
    #[test]
    fn test_equality_delete_unreachable_column() -> Result<(), anyhow::Error> {
        let schema = Arc::new(
            Schema::builder()
                .with_schema_id(1)
                .with_fields(vec![
    ...
    #[test]
    fn test_equality_delete_with_primitive_type() -> Result<(), anyhow::Error> {
        let temp_dir = TempDir::new().unwrap();
        let file_io = FileIO::local();
        let location_gen = DefaultLocationGenerator::with_data_location(
            temp_dir.path().to_str().unwrap().to_string(),
    ...
    #[test]
    fn test_equality_delete_with_nullable_field() -> Result<(), anyhow::Error> {
        // prepare data
