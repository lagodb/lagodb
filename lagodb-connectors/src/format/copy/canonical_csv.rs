...
mod canonical_csv_tests {
    use super::{CanonicalCsv, CanonicalCsvRow};
    use pgrx::pg_test;

    #[pg_test]
    fn writer_quotes_protocol_sentinels_and_csv_metacharacters() {
        for (value, expected) in [
            (b"".as_slice(), b"\"\"".as_slice()),
            (br"\N".as_slice(), br#""\N""#.as_slice()),
            (br"\.".as_slice(), br#""\.""#.as_slice()),
            (b"a,b".as_slice(), br#""a,b""#.as_slice()),
            (b"a\"b".as_slice(), br#""a""b""#.as_slice()),
        ] {
            let mut output = Vec::new();
            CanonicalCsv::write_field(&mut output, value);
            assert_eq!(output, expected);
        }
    }

    #[pg_test]
    fn parser_reuses_storage_and_preserves_null_empty_and_escaped_fields() {
        let mut row = CanonicalCsvRow::new();
        row.parse(br#"\N,"",plain,"a,b","a""b","#.as_slice(), 6)
            .unwrap();
        let fields = row
            .fields()
            .map(|field| field.map(|value| value.to_bytes().to_vec()))
            .collect::<Vec<_>>();
        assert_eq!(
            fields,
            vec![
    ...
    #[pg_test]
    fn parser_rejects_invalid_framing_and_width() {
        let mut row = CanonicalCsvRow::new();
        assert!(row.parse(br#""unterminated"#.as_slice(), 1).is_err());
        assert!(row.parse(b"one,two", 1).is_err());
        assert!(row.parse(b"nul\0byte", 1).is_err());
    }
}
