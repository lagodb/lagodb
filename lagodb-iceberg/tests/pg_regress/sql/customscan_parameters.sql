...
-- (`EXACT_ALLOWLIST` in `lagodb-iceberg/src/customscan/classifier.rs`),
...
--   - Inner: `Custom Scan (lagodb-iceberg) on customscan_ord_lake`.
--   - On the inner, in default TEXT, a `Pushed Filter: (k = o.id)` line
--     — the ordinary join predicate `l.k = o.id` is the pushed (remote)
--     predicate realized at the CustomScan node. It is Exact and stripped
--     from `plan.qual`, so it does NOT
--     appear as a residual `Filter:` line above the scan, which is the
--     structural evidence that the
...
-- `lagodb-iceberg/src/customscan/classifier.rs`), so each leg of the
...
-- `Custom Scan (lagodb-iceberg)` that, in default TEXT, carries a
...
-- `lagodb-iceberg/src/customscan/classifier.rs`. The matching outer
...
-- ============================================================================
-- NON-pushable join clause → unparameterized (Plain) variant wins
-- ============================================================================
...
-- Under `force`, EXPLAIN must show:
--   - A `Nested Loop` plan node.
--   - Inner: `Custom Scan (lagodb-iceberg) on customscan_var_lake l`.
...
-- Result-set parity. With outer.id ∈ {1, 5, 105} and the join
...
-- ============================================================================
-- PUSHABLE join clause → parameterized (JoinParameterized) variant wins
-- ============================================================================
-- An ordinary
...
-- The preceding non-pushable clause `(l.k + 1) = o.id` is NOT a bare mergejoinable
...
-- subquery, which must keep the LATERAL shape to present that clause at all.
-- The pushable clause IS a bare equality, so the ordinary
...
-- is the cheaper plan. (The non-pushable case keeps its own small
...
--   - Inner: `Custom Scan (lagodb-iceberg) on customscan_var_join_lake l`.
--   - On the inner, in default TEXT, a `Pushed Filter: (k = o.id)` line
...
-- Result-set parity. Each outer.id matches exactly one lake
...
-- `lagodb-iceberg/src/customscan/classifier.rs`), so a `WHERE id = $1`
...
-- ============================================================================
-- PARAM_EXTERN via PREPARE / EXECUTE
...
-- `paramInfo->params[$1]`. The framework initializes its binding
-- expressions with `ExecInitExprList` and evaluates them
-- through `ExecEvalExprSwitchContext`. PostgreSQL therefore owns
-- `PARAM_EXTERN` fetching and type validation; the framework binds the
-- evaluated values to the planned filters.
...
-- switch to a generic plan below.
-- ============================================================================
...
-- Bound value lives in file 2 (NULL-bearing file). 105 is NOT
...
-- Bound value matches one of the NULL ids of file 2 (119 IS one
...
-- PG generic-plan path — PG normally switches to the generic
...
-- `BeginCustomScan` and PostgreSQL expression evaluation are exercised.
-- After the switch, run one more `EXECUTE` on each side and let pg_regress diff
...
-- ============================================================================
-- Nestloop join with `lake.id = outer.id` on the inner side
...
-- Five-row outer, every outer.id matches a unique inner row.
SET lagodb.customscan_mode = 'force';
WITH outer_rel(id) AS (
    VALUES (1), (25), (100), (1000), (1050)
)
...
-- Outer tuple whose id has NO matching lake row — the inner
...
-- Outer with repeated values — exercises rescan-with-same-value.
...
-- state, and the framework never compares Datum values for equality to
-- short-circuit; the executor's `chgParam` signal is the correctness-bearing
-- trigger.
SET lagodb.customscan_mode = 'force';
WITH outer_rel(id) AS (
...
-- Nestloop where the pushed inner predicate is a CONSTANT. The
...
-- `Param` references. Therefore its binding parameter set is empty and
-- cannot overlap `chgParam` on any rescan. The framework must not rebind
-- or rebuild pruning state; it only reopens the cursor. We cannot observe
-- "didn't rebind" from SQL, but we can assert the
-- result-set correctness still matches the baseline 
-- The chgParam gate is correctness-preserving regardless of
-- which path it picks).
-- This case complements the changing-parameter rescans above, where the gate is always
...
-- Plan guards: this constant predicate differs from the changing-parameter cases
-- (constant `lake.id = 25` is planned Exact and pushed; the
-- outer-dependent `(lake.id + outer.unrelated) > 0` is the local
-- residual). So the
-- The nestloop plan guard above does not certify this planned-filter outcome — we
...
-- PARAM_EXEC changes from a representable ConservativePruning value to
-- a value that cannot be encoded as an Iceberg Datum. The pushed predicate is
-- a ConservativePruning OR, so `ValueNotRepresentable` must omit it from the
-- current bound set, clearing the Iceberg row_filter and relying on the
-- residual qual. Keeping the previous
...
-- ============================================================================
-- Plan guards — the inner lake scan carries BOTH a PARAM_EXTERN
...
-- `Custom Scan (lagodb-iceberg) on customscan_collide_lake`. In default
...
-- ============================================================================
-- Row-set parity for the colliding-kind scan
...
-- genuine `PARAM_EXTERN` at `BeginCustomScan`, while each outer tuple drives
-- a `ReScanCustomScan` that re-resolves the `PARAM_EXEC` value.
...
-- `$1 = 1`: tag = 1 rows that also match a correlated outer.sel.
SET lagodb.customscan_mode = 'force';
...
-- `$1 = 0`: a DIFFERENT EXTERN value selects a different `tag` class.
-- If the EXTERN value had been collapsed onto the EXEC slot (or vice
-- versa), the two EXTERN values would not differ in the way the baseline says
...
-- `$1 = 2`: third `tag` class, exercising file 1 (tag = g % 3) where
...
-- ============================================================================
-- ReScan after the PARAM_EXEC value changes
-- The nestloop above already rescans the inner CustomScan once per outer
-- tuple, so the parity case exercises ReScan implicitly. This case makes the
...
-- `$1 = 0`: across the rescans, only rows whose tag = 0 AND whose sel
...
-- `$1 = 1`: same rescan sequence, different EXTERN value. Demonstrates
...
-- (`EXACT_ALLOWLIST` in `lagodb-iceberg/src/customscan/classifier.rs`), so a
-- `WHERE id = $1` clause whose `$1` is `PARAM_EXTERN`, or `PARAM_EXEC` after
-- `replace_nestloop_params`, is pushed for
...
-- The outer relation for the nestloop case. Row (1, 'one') supplies a non-NULL match;
...
-- Plan guard: the `force` plan must be a CustomScan with Exact pushdown
...
-- EXECUTE p(NULL): a strict `id = NULL` comparison is UNKNOWN, which a
-- WHERE context filters out, so the correct result is 0 rows. The expected
-- `.out` encodes 0 rows. Before the fix, the `force` block raised `ERROR`
...
-- ============================================================================
-- Nestloop whose outer side supplies NULL to a PARAM_EXEC param
-- ============================================================================
...
-- CustomScan is the cheaper plan. (The prepared-statement case keeps its own small
...
-- Plan guard: `force` => Nested Loop with a parameterized CustomScan on
...
-- Result parity: the outer NULL row contributes 0 rows (its inner
...
-- that single row. Before the fix, the `force` block raised `ERROR` when
...
-- ============================================================================
-- Mixed conjunction `id = $1 AND amount > 0` with $1 NULL
-- ============================================================================
...
-- (`customscan_null_param_mixed`) to stay independent of the other NULL-parameter
-- `customscan_null_param_lake` schema. The table is created and dropped
-- entirely within this self-contained block.
-- As in the prepared-statement case, `plan_cache_mode = force_generic_plan` keeps `$1` a runtime
...
-- EXECUTE with `$1` NULL: `id = NULL` is UNKNOWN, so the `AlwaysFalse`
...
-- ============================================================================
-- Non-NULL parity regression guard
-- ============================================================================
-- Preservation guard: the SAME prepared `WHERE id = $1` shape as the NULL case, but
...
-- Reuses the `customscan_null_param_lake` table (id + payload, rows
...
-- (the same shape exercised with NULL above), confirming the non-NULL path
...
-- EXECUTE p(5): `id = 5` is a non-NULL strict comparison, pushed as an
