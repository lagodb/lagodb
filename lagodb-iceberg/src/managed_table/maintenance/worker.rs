...
use crate::managed_table::gucs::{
    auto_maintenance_enabled, auto_maintenance_max_tables, auto_maintenance_naptime,
};
...
impl SchedulerPolicy {
    fn configured() -> Self {
        Self {
            naptime: auto_maintenance_naptime(),
            max_tables: auto_maintenance_max_tables(),
        }
    }
...
fn maintain_relation(
    ...
    if expected_am != Some(relation.access_method_oid()) {
        IcebergMetadata::finish_maintenance(
            relid,
            metadata_location,
            MaintenanceScheduleUpdate::CompleteIfDueMatches(Some(candidate.due_at)),
        )
        .map_err(PgReportError::from_domain_error)?;
