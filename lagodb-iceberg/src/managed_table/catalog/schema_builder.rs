...
impl PgType {
    ...
    fn primitive_type(&self) -> IcebergResult<Type> {
        ...
        match pg_oid {
            ...
            // string and jsonb to binary as a lagodb-iceberg private codec:
            // jsonb bytes are PostgreSQL-internal varlena, not a portable
            // Iceberg JSON encoding.
            //
            // TODO: Replace the private jsonb binary codec with an Iceberg V3
            // Variant mapping after iceberg-rust completes the Variant data
            // path (non-shredded read/write and shredded read) and establishes
            // its parquet-rs integration. parquet-rs Variant support remains
            // behind `variant_experimental`. Track iceberg-rust#2411 and
            // arrow-rs#8480.
