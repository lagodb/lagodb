...
//! lagodb-iceberg had its own mapping that disagreed with iceberg-lite (notably
...
/// Canonical PostgreSQL type emitted when an Iceberg schema creates a local
/// column definition. Live relations may use another compatible type from the
/// same [`PgColumnType`] bucket; scan binding still validates that live OID.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct IcebergPostgresType {
    oid: pg_sys::Oid,
    typmod: i32,
    column_type: PgColumnType,
}

impl IcebergPostgresType {
...
pub(crate) trait IcebergTypeExt {
    ...
    fn postgres_type(&self) -> Option<IcebergPostgresType>;
}
...
impl IcebergTypeExt for Type {
    ...
    fn pg_column_type(&self) -> Option<PgColumnType> {
        self.postgres_type().map(IcebergPostgresType::column_type)
    }
        ...
        let postgres = match self {
            Type::Primitive(p) => match p {
                PrimitiveType::Boolean => {
                    IcebergPostgresType::plain(pg_sys::BOOLOID, PgColumnType::Bool)
                }
                PrimitiveType::Int => {
                    IcebergPostgresType::plain(pg_sys::INT4OID, PgColumnType::Int4)
                }
                PrimitiveType::Long => {
                    IcebergPostgresType::plain(pg_sys::INT8OID, PgColumnType::Int8)
                }
                PrimitiveType::Float => IcebergPostgresType::plain(
                    pg_sys::FLOAT4OID,
                    PgColumnType::Float4,
                ),
                PrimitiveType::Double => IcebergPostgresType::plain(
                    pg_sys::FLOAT8OID,
                    PgColumnType::Float8,
                ),
                PrimitiveType::Decimal { precision, scale } => {
                    return Some(IcebergPostgresType {
                ...
                PrimitiveType::Date => {
                    IcebergPostgresType::plain(pg_sys::DATEOID, PgColumnType::Date)
                }
                PrimitiveType::Time => {
                    IcebergPostgresType::plain(pg_sys::TIMEOID, PgColumnType::Time)
                }
                PrimitiveType::Timestamp | PrimitiveType::TimestampNs => {
                    IcebergPostgresType::plain(
                        pg_sys::TIMESTAMPOID,
                ...
                PrimitiveType::String => {
                    IcebergPostgresType::plain(pg_sys::TEXTOID, PgColumnType::Text)
                }
                ...
                PrimitiveType::Uuid => {
                    IcebergPostgresType::plain(pg_sys::UUIDOID, PgColumnType::Uuid)
                }
                PrimitiveType::Fixed(_) | PrimitiveType::Binary => {
                    IcebergPostgresType::plain(pg_sys::BYTEAOID, PgColumnType::Bytea)
                }
            },
            ...
            Type::List(list) => match list.element_field.field_type.as_ref() {
                ...
                Type::Primitive(PrimitiveType::Int) => IcebergPostgresType::plain(
                    pg_sys::INT4ARRAYOID,
                    PgColumnType::Array(pg_sys::INT4OID),
                ),
                Type::Primitive(PrimitiveType::Long) => IcebergPostgresType::plain(
                    pg_sys::INT8ARRAYOID,
                    PgColumnType::Array(pg_sys::INT8OID),
                ),
                Type::Primitive(PrimitiveType::Float) => IcebergPostgresType::plain(
                    pg_sys::FLOAT4ARRAYOID,
                    PgColumnType::Array(pg_sys::FLOAT4OID),
                ),
                Type::Primitive(PrimitiveType::Double) => IcebergPostgresType::plain(
                    pg_sys::FLOAT8ARRAYOID,
                    PgColumnType::Array(pg_sys::FLOAT8OID),
                ),
                Type::Primitive(PrimitiveType::String) => IcebergPostgresType::plain(
                    pg_sys::TEXTARRAYOID,
...
/// Reject types that lagodb-iceberg's mutation/scan paths cannot handle.
///
...
impl ValidateSupported for PrimitiveType {
    /// Reject primitive shapes lagodb-iceberg cannot encode into Arrow even
...
mod tests {
    ...
    fn iceberg_schema_to_arrow_schema_binary_types() {
        ...
        // lagodb-iceberg delegates the Iceberg -> Arrow type table to
