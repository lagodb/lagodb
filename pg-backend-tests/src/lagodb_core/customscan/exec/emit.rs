...
mod tests {
    ...
    use crate::lagodb_core::customscan::support::{
        TestScanState, impl_reject_all_filters,
    };
    use lagodb_core::api::AmResult;
    use lagodb_core::batch::ScanBatchDriver;
    ...
    use lagodb_core::handles::{RelationHandle, ScanDirection};
    use lagodb_core::tuple::{Cell, Row, RowDatumCodec, SlotColumns};
    use pgrx::pg_sys;
    use pgrx::pg_test;
    ...
    const HANDLE_EMIT_TEXT: &str =
        "lagodb-emit-row-varlena-lands-in-the-per-tuple-context";
    ...
    impl LagodbCustomScanProvider for HandleAccessorProvider {
    ...
    struct ScanFixture<P: LagodbCustomScanProvider> {
    ...
    unsafe fn synth_scan_wrapper<P: LagodbCustomScanProvider>(
        state: P::State,
    ) -> ScanFixture<P> {
        unsafe {
            ...
            let per_tuple_ctx = pg_sys::AllocSetContextCreateExtended(
                ...
                c"lagodb test per-tuple ctx".as_ptr(),
    ...
    const EMIT_COLUMNS_TEXT: &str =
        "lagodb-emit-columns-varlena-lands-in-the-per-tuple-context";
    ...
    impl LagodbCustomScanProvider for EmitColumnsProvider {
