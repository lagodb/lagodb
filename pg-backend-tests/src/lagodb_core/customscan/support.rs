...
use lagodb_core::customscan::provider::{CustomScanError, LagodbCustomScanProvider};
use lagodb_core::customscan::state::{
...
macro_rules! impl_reject_all_filters {
    ($provider:ty) => {
        impl lagodb_core::expr::pushdown::FilterPushdown for $provider {
            type Planner =
                $crate::lagodb_core::customscan::support::RejectAllFilterPlanner;
            type PlannedPredicate = ();
            type BoundPredicate = ();
            type Error = lagodb_core::customscan::provider::CustomScanError;

            fn begin_filter_planning(
                _context: &lagodb_core::expr::pushdown::FilterPlanningContext,
            ) -> Result<Self::Planner, Self::Error> {
                Ok($crate::lagodb_core::customscan::support::RejectAllFilterPlanner)
            }

            fn encode_planned(
                _predicate: &Self::PlannedPredicate,
                _writer: &mut lagodb_core::plan_data::PlanDataWriter,
            ) -> Result<(), Self::Error> {
                Ok(())
            }

            fn decode_planned(
                _reader: &mut lagodb_core::plan_data::PlanDataReader<'_>,
            ...
            fn bind_filter(
                ...
                Self::Error,
            > {
                Ok(lagodb_core::expr::pushdown::FilterBindResult::ValueNotRepresentable)
            }
        }
    };
}
...
pub(crate) struct TestScanState<P: LagodbCustomScanProvider> {
    wrapper: *mut pg_sys::CustomScanState,
    _provider: PhantomData<fn() -> P>,
}

impl<P: LagodbCustomScanProvider> TestScanState<P> {
