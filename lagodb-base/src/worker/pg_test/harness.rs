...
impl WorkerHarness {
    ...
    fn maintenance_key(&self) -> WorkerKey {
        let worker_id = Spi::get_one::<i32>(&format!(
            "SELECT worker_id FROM lagodb.worker_status WHERE database_oid = {} AND extension_name = 'lagodb_base' AND worker_name = 'maintenance'",
            self.database_oid
        ))
    ...
    pub(super) fn worker_pid(&self) -> i32 {
        Spi::get_one::<i32>(&format!(
            "SELECT pid FROM lagodb.worker_status WHERE database_oid = {} AND extension_name = 'lagodb_base' AND worker_name = 'maintenance'",
            self.database_oid
        ))
    ...
    pub(super) fn supervisor_pid(&self) -> i32 {
        Spi::get_one::<i32>(
            "SELECT pid FROM lagodb.process_status WHERE process_kind = 'supervisor'",
        )
    ...
    pub(super) fn coordinator_pid(&self) -> i32 {
        Spi::get_one::<i32>(&format!(
            "SELECT pid FROM lagodb.process_status WHERE process_kind = 'coordinator' AND database_oid = {}",
            self.database_oid
        ))
    ...
    pub(super) fn wait_for_coordinator(&self) {
        self.wait_for_query(
            ...
            &format!(
                "SELECT EXISTS (SELECT FROM lagodb.process_status WHERE process_kind = 'coordinator' AND database_oid = {} AND state = 'running' AND pid IS NOT NULL)",
                self.database_oid
            ),
        );
    }
    ...
    pub(super) fn wait_for_worker(&self, predicate: &str) {
        self.wait_for_query(
            predicate,
            &format!(
                "SELECT EXISTS (SELECT FROM lagodb.worker_status WHERE database_oid = {} AND extension_name = 'lagodb_base' AND worker_name = 'maintenance' AND ({predicate}))",
                self.database_oid
            ),
        );
    }
    ...
    pub(super) fn wait_for_worker_at_connection_point(&self) {
        self.wait_for_query(
            ...
            &format!(
                concat!(
                    "SELECT EXISTS (SELECT FROM lagodb.worker_status AS worker ",
    ...
    pub(super) fn wait_for_query(&self, description: &str, query: &str) {
        ...
        loop {
            ...
            if Instant::now() >= deadline {
                ...
                "SELECT jsonb_build_object('workers', coalesce((SELECT jsonb_agg(to_jsonb(status)) FROM lagodb.worker_status AS status), '[]'::jsonb), 'processes', coalesce((SELECT jsonb_agg(to_jsonb(status)) FROM lagodb.process_status AS status), '[]'::jsonb))::text",
                )
