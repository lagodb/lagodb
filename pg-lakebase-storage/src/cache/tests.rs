...
fn maps_read_to_chunks() {
    ...
    assert_eq!(
        cache.chunks_for_read(u64::MAX - 2, 4, u64::MAX),
        vec![u64::MAX / 4]
    );
}
...
async fn cleanup_deletes_unclaimed_complete_and_partial_candidates() {
    ...
    cache
        .orphan_candidates
        .record_file_candidate(complete_path.clone());
    cache
        .orphan_candidates
    ...
    assert!(
        tokio::fs::try_exists(cache.complete_path(&live_key).unwrap())
            .await
            .unwrap()
    );
...
fn test_cache_dir() -> PathBuf {
    ...
    PathBuf::from("/tmp").join(format!(
        "pg-lakebase-storage-cache-test-{}-{stamp}-{id}",
        std::process::id()
    ))
}
...
async fn best_effort_invalidate_removes_cached_small_entry() {
    ...
    assert!(
        matches!(outcome, BestEffortInvalidateOutcome::Removed { bytes } if bytes == 3)
    );
    assert!(cache.index().get_meta(&key).await.unwrap().is_none());
}
