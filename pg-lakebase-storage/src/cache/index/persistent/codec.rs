...
use crate::cache::meta::{CacheState, CachedObjectMeta, CachedResidency, ObjectIdentity};
...
pub(super) fn encode_meta(meta: &CachedObjectMeta) -> Vec<u8> {
    ...
    out.write_u64::<BigEndian>(meta.size()).expect("infallible write to Vec");
    match meta.etag() {
        Some(etag) => {
    ...
    out.write_u8(meta.cache_state().as_u8()).expect("infallible write to Vec");
    out.write_u64::<BigEndian>(meta.last_access_ns).expect("infallible write to Vec");
    out.write_u64::<BigEndian>(meta.generation).expect("infallible write to Vec");
    match meta.residency() {
        CachedResidency::Small { bytes } => out.write_u64::<BigEndian>(*bytes).expect("infallible write to Vec"),
...
pub(super) fn decode_meta(bytes: &[u8]) -> StorageResult<CachedObjectMeta> {
    ...
    let etag = match input.read_u8()? {
        ...
        other => return Err(StorageError::cache(format!("invalid etag tag {other}"))),
    };
...
fn write_string(out: &mut Vec<u8>, value: &str) {
    out.write_u32::<BigEndian>(value.len() as u32).expect("infallible write to Vec");
...
fn read_string(input: &mut Cursor<&[u8]>) -> StorageResult<String> {
    ...
    String::from_utf8(bytes)
        .map_err(|error| StorageError::cache_source("invalid utf-8 string in cache metadata", error))
}

fn ensure_remaining(input: &Cursor<&[u8]>, wanted: usize, field: &'static str) -> StorageResult<()> {
    let total = input.get_ref().len();
    let position = usize::try_from(input.position())
        .map_err(|_| StorageError::cache("cache metadata decode cursor position overflow"))?;
    if position > total {
        return Err(StorageError::cache("cache metadata decode cursor past end of payload"));
    }
