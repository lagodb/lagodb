...
use super::traits::{ensure_eof, get_u16, WireDecode, WireEncode};
...
impl WireEncode for WireRequestPayload {
    fn encode(&self, out: &mut impl BufMut) -> StorageResult<()> {
        match self {
            ...
            Self::StageCreate { store_id, bucket, key } => {
                out.put_u16(WireOp::StageCreate.code());
                store_id.encode(out)?;
                bucket.encode(out)?;
                key.encode(out)
            },
            Self::Commit { store_id, bucket, key } => {
                out.put_u16(WireOp::Commit.code());
                store_id.encode(out)?;
                bucket.encode(out)?;
                key.encode(out)
            },
            Self::Abort { store_id, bucket, key } => {
            ...
            Self::InvalidateObjectCache { store_id, bucket, key } => {
                out.put_u16(WireOp::InvalidateObjectCache.code());
                store_id.encode(out)?;
                bucket.encode(out)?;
                key.encode(out)
            },
            Self::Delete { store_id, bucket, key } => {
                out.put_u16(WireOp::Delete.code());
                store_id.encode(out)?;
                bucket.encode(out)?;
                key.encode(out)
            },
            Self::DeletePrefix { store_id, bucket, prefix } => {
