...
async fn stage_commit_then_read_back_on(kind: CacheIndexKind) {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let mut staging = client.stage(STORE_ID, TEST_BUCKET, "staged/object.bin").unwrap();
        staging.write(payload).unwrap();
        staging.sync().unwrap();
        drop(staging);

        let info = client.commit(STORE_ID, TEST_BUCKET, "staged/object.bin").unwrap();
        ...
        let mut file = client.open(STORE_ID, TEST_BUCKET, "staged/object.bin").unwrap();
...
async fn stage_abort_does_not_upload() {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let mut staging = client.stage(STORE_ID, TEST_BUCKET, "aborted/file.txt").unwrap();
        staging.write(b"these bytes should never reach MinIO").unwrap();
        drop(staging);

        client.abort(STORE_ID, TEST_BUCKET, "aborted/file.txt").unwrap();
...
async fn commit_overwrites_and_readback_sees_new_data_on(kind: CacheIndexKind) {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let mut staging = client.stage(STORE_ID, TEST_BUCKET, "overwrite.txt").unwrap();
        staging.write(replacement).unwrap();
        drop(staging);

        let info = client.commit(STORE_ID, TEST_BUCKET, "overwrite.txt").unwrap();
        assert_eq!(info.size, replacement.len() as u64);

        client.invalidate_object_cache(STORE_ID, TEST_BUCKET, "overwrite.txt").unwrap();

        let mut f = client.open(STORE_ID, TEST_BUCKET, "overwrite.txt").unwrap();
        assert_eq!(f.size(), replacement.len() as u64);
        f.seek(SeekFrom::Start(0));
        assert_eq!(f.read(replacement.len() as u32).unwrap(), replacement.as_ref());
        f.close().unwrap();
    })
    .await
    .unwrap();
...
async fn delete_removes_object_on(kind: CacheIndexKind) {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        client.delete(STORE_ID, TEST_BUCKET, "deleteme.txt").unwrap();
