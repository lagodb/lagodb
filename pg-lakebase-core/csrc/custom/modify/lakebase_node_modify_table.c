...
#define ExecComputeStoredGenerated LakebaseExecComputeStoredGenerated
#define ExecGetUpdateNewTuple LakebaseExecGetUpdateNewTuple
#define ExecInitMergeTupleSlots LakebaseExecInitMergeTupleSlots
#define ExecLookupResultRelByOid LakebaseExecLookupResultRelByOid
#define ExecInitModifyTable LakebaseExecInitModifyTable
#define ExecEndModifyTable LakebaseExecEndModifyTable
#define ExecReScanModifyTable LakebaseExecReScanModifyTable
/* LAKEBASE END */
...
typedef struct ModifyTableContext
{
	...
	/* LAKEBASE BEGIN: query-lifetime Rust bridge and wholerow-derived OLD slot. */
	LakebaseModifyBridge *lakebase_bridge;
	AttrNumber	lakebase_wholerow_attno;
	ResultRelInfo *lakebase_result_rel_info;
	void	   *lakebase_relation_state;
	TupleTableSlot *lakebase_old_slot;
	/* LAKEBASE END */
...
/* LAKEBASE BEGIN: narrow bridge helper. */
static void *
LakebaseRelationState(ModifyTableContext *context,
					  ResultRelInfo *resultRelInfo)
{
	LakebaseModifyBridge *bridge = context->lakebase_bridge;

	if (context->lakebase_result_rel_info == resultRelInfo)
		return context->lakebase_relation_state;

	context->lakebase_result_rel_info = resultRelInfo;
	context->lakebase_relation_state =
		bridge != NULL && bridge->resolve_relation != NULL ?
		bridge->resolve_relation(bridge->state, resultRelInfo) : NULL;
	if (context->lakebase_relation_state != NULL &&
...
static bool
LakebaseSupportsRelation(ModifyTableContext *context,
						 ResultRelInfo *resultRelInfo)
{
	return LakebaseRelationState(context, resultRelInfo) != NULL;
}
 ...
 */
static void
LakebaseSetSelfModifiedFailureData(ModifyTableContext *context,
		   ...
		   ResultRelInfo **insert_destrel)
{
	...
	vlock:
		...
		else
		{
			...
			/* LAKEBASE BEGIN: final INSERT slot mutation boundary. */
			if (LakebaseSupportsRelation(context, resultRelInfo))
			{
				LakebaseModifyBridge *bridge = context->lakebase_bridge;

				bridge->insert(LakebaseRelationState(context, resultRelInfo), slot,
		...
		resultRelInfo->ri_TrigDesc->trig_insert_after_row)
	{
		...
		context->lakebase_bridge->preserve_trigger_row(
			LakebaseRelationState(context, resultRelInfo), slot, &trigger_tid);
				   ...
				   TupleTableSlot **epqreturnslot, TM_Result *result)
{
	...
	{
		if (LakebaseSupportsRelation(context, resultRelInfo) && oldtuple == NULL)
			elog(ERROR, "Lakebase DELETE trigger input has no wholerow");
			  ...
			  ItemPointer tupleid, bool changingPart)
{
	...
	/* LAKEBASE BEGIN: identity-only storage mutation boundary. */
	if (LakebaseSupportsRelation(context, resultRelInfo))
	{
		LakebaseModifyBridge *bridge = context->lakebase_bridge;
		LakebaseMutationResult mutation;

		if (tupleid == NULL)
			elog(ERROR, "Lakebase DELETE has no synthetic ctid");
		mutation = bridge->delete_(LakebaseRelationState(context, resultRelInfo),
								  tupleid,
								  estate->es_output_cid,
								  estate->es_snapshot,
		...
		{
			case LAKEBASE_MUTATION_APPLIED:
				return TM_Ok;
			case LAKEBASE_MUTATION_SELF_MODIFIED:
				LakebaseSetSelfModifiedFailureData(context,
									   mutation.modifying_cid);
				return TM_SelfModified;
			case LAKEBASE_MUTATION_DELETED:
				return TM_Deleted;
		}
		elog(ERROR, "unknown Lakebase DELETE outcome");
	}
	/* LAKEBASE END */
...
ldelete:
	...
	if (processReturning && resultRelInfo->ri_projectReturning)
	{
		...
		else
		{
			...
			{
				if (LakebaseSupportsRelation(context, resultRelInfo))
					elog(ERROR, "Lakebase DELETE RETURNING input has no wholerow");
...
lreplace:
	 ...
	 * transactions.
	 */
	/* LAKEBASE BEGIN: identity/OLD/final-NEW slot mutation boundary. */
	if (LakebaseSupportsRelation(context, resultRelInfo))
	{
		LakebaseModifyBridge *bridge = context->lakebase_bridge;
		LakebaseMutationResult mutation;

		if (context->lakebase_old_slot == NULL)
			elog(ERROR, "Lakebase UPDATE has no OLD slot");
		if (tupleid == NULL)
			elog(ERROR, "Lakebase UPDATE has no synthetic ctid");
		mutation = bridge->update(LakebaseRelationState(context, resultRelInfo),
								tupleid,
								context->lakebase_old_slot, slot,
								estate->es_output_cid,
								estate->es_snapshot,
								estate->es_crosscheck_snapshot,
								true);
		...
		switch (mutation.outcome)
		{
			...
			case LAKEBASE_MUTATION_DELETED:
				...
				break;
			default:
				elog(ERROR, "unknown Lakebase UPDATE outcome");
		}
	}
	else
	{
				   ...
				   HeapTuple oldtuple, TupleTableSlot *slot)
{
		...
		resultRelInfo->ri_TrigDesc->trig_update_after_row)
	{
		...
		if (oldtuple == NULL || context->lakebase_old_slot == NULL)
			elog(ERROR, "Lakebase AFTER UPDATE trigger has no OLD row");
		...
		ExecForceStoreHeapTuple(oldtuple, trigger_old_slot, false);
		context->lakebase_bridge->prepare_update_trigger_rows(
			context->lakebase_bridge->state, resultRelInfo, resultRelInfo,
			trigger_old_slot, slot, &prepared);
		if (!ItemPointerIsValid(&prepared.old_tid) ||
			!ItemPointerIsValid(&prepared.new_tid))
			elog(ERROR, "Lakebase UPDATE trigger rows were not preserved");
		saved_old_tid = oldtuple->t_self;
								   ...
								   TupleTableSlot *newslot)
{
	...
	List	   *ancestorRels;
	HeapTuple	lakebase_oldtuple = NULL;
	bool		should_free = false;
	bool		source_is_lakebase;
	bool		destination_is_lakebase;
	...
	/*
	 * LAKEBASE BEGIN: preserve each custom-AM side under the exact leaf
	 * relation PostgreSQL will use when the queued trigger event is fired.
	 * Native heap sides retain their physical TIDs.
	 */
	source_is_lakebase = LakebaseSupportsRelation(context, sourcePartInfo);
	destination_is_lakebase = LakebaseSupportsRelation(context, destPartInfo);
	if (source_is_lakebase || destination_is_lakebase)
	{
		LakebasePreparedUpdateTriggerRows prepared;
		...
		ItemPointerSetInvalid(&prepared.old_tid);
		ItemPointerSetInvalid(&prepared.new_tid);
		if (source_is_lakebase)
		{
			if (context->lakebase_old_slot == NULL)
				elog(ERROR, "Lakebase cross-partition UPDATE has no OLD slot");
			lakebase_oldtuple =
				ExecFetchSlotHeapTuple(context->lakebase_old_slot,
								   true, &should_free);
		}
		context->lakebase_bridge->prepare_update_trigger_rows(
			context->lakebase_bridge->state, sourcePartInfo, destPartInfo,
			source_is_lakebase ? context->lakebase_old_slot : NULL,
			newslot, &prepared);

		if (source_is_lakebase)
		{
			if (!ItemPointerIsValid(&prepared.old_tid))
				elog(ERROR, "Lakebase cross-partition UPDATE did not preserve OLD");
			saved_old_tid = lakebase_oldtuple->t_self;
			lakebase_oldtuple->t_self = prepared.old_tid;
		}
		if (destination_is_lakebase)
		{
			if (!ItemPointerIsValid(&prepared.new_tid))
				elog(ERROR, "Lakebase cross-partition UPDATE did not preserve NEW");
			newslot->tts_tid = prepared.new_tid;
		}

		ExecARUpdateTriggers(context->estate,
						 rootRelInfo, sourcePartInfo, destPartInfo,
						 lakebase_oldtuple != NULL ? NULL : tupleid,
						 lakebase_oldtuple, newslot, NIL, NULL, true);
		if (destination_is_lakebase)
			newslot->tts_tid = saved_new_tid;
		if (source_is_lakebase)
			lakebase_oldtuple->t_self = saved_old_tid;
	}
	else
	{
	...
	/* LAKEBASE END */
	if (should_free)
		heap_freetuple(lakebase_oldtuple);
}
				 ...
				 bool *matched)
{
	...
	if (oldtuple != NULL)
	{
		...
		if (LakebaseSupportsRelation(context, resultRelInfo))
			context->lakebase_old_slot = resultRelInfo->ri_oldTupleSlot;
		/* LAKEBASE END */
	}
	else
	{
...
lmerge_matched:
	...
	foreach(l, actionStates)
	{
		...
		{
			case CMD_UPDATE:
				/* LAKEBASE BEGIN: MERGE actions consume the carried OLD tuple. */
 ...
 * ----------------------------------------------------------------
 */
/* LAKEBASE BEGIN: pass the execution-owned Rust bridge through upstream flow. */
static TupleTableSlot *
ExecModifyTableWithBridge(PlanState *pstate, LakebaseModifyBridge *bridge)
{
/* LAKEBASE END */
	...
	/* LAKEBASE BEGIN */
	context.lakebase_bridge = bridge;
	context.lakebase_wholerow_attno =
		bridge != NULL && bridge->wholerow_attno != NULL ?
		bridge->wholerow_attno(bridge->state) : InvalidAttrNumber;
	context.lakebase_result_rel_info = NULL;
	context.lakebase_relation_state = NULL;
	context.lakebase_old_slot = NULL;
	/* LAKEBASE END */
	...
	for (;;)
	{
		...
		tupleid = NULL;
		oldtuple = NULL;
		/* LAKEBASE BEGIN: reset the per-row wholerow-derived OLD slot. */
		context.lakebase_old_slot = NULL;
		/* LAKEBASE END */
			...
			operation == CMD_MERGE)
		{
			...
			/* LAKEBASE BEGIN: reconstruct OLD from PostgreSQL wholerow. */
			if (LakebaseSupportsRelation(&context, resultRelInfo))
			{
				AttrNumber	wholerow_attno = context.lakebase_wholerow_attno;
				...
				if (!AttributeNumberIsValid(resultRelInfo->ri_RowIdAttNo))
				{
					...
					elog(ERROR, "Lakebase mutation input has no ctid row identity");
				}
				...
				if (isNull)
				{
					...
					elog(ERROR, "Lakebase matched mutation row has NULL ctid");
				}
				...
				if (AttributeNumberIsValid(wholerow_attno))
				{
					...
					if (isNull)
						elog(ERROR, "Lakebase matched mutation row has NULL wholerow");
					...
					elog(ERROR, "Lakebase UPDATE/MERGE input has no wholerow");
			}
			/* LAKEBASE END */
		...
		switch (operation)
		{
			...
			case CMD_UPDATE:
				...
				if (oldtuple != NULL)
				{
					...
					if (LakebaseSupportsRelation(&context, resultRelInfo))
						context.lakebase_old_slot = oldSlot;
					/* LAKEBASE END */
				}
				else
				{
...
/* LAKEBASE BEGIN: standard copy-only entry plus extension entry. */
static TupleTableSlot *
...
TupleTableSlot *
lakebase_exec_modify_table(ModifyTableState *mtstate,
						   LakebaseModifyBridge *bridge)
{
	if (mtstate == NULL || bridge == NULL)
		elog(ERROR, "Lakebase ModifyTable executor received NULL state");
