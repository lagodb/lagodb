# PostgreSQL 17 ANALYZE ReadStream baseline
...
`read_stream_pg17.c` is an extension-local, narrow private-layout adapter. It
...
The C translation unit deliberately rejects every `PG_VERSION_NUM` other than
17.10. Because PostgreSQL module magic checks only the major version, the
adapter also checks the running backend's `server_version_num` before touching
the private layout. Before adopting a new PostgreSQL 17 minor release, update
both guards only after completing this audit:
...
from that same stream. Those checks reject inconsistent sampler semantics and
unexpected callers. They do not make an unreviewed private layout safe to
dereference; that safety comes from the exact build/runtime version guards and
the minor-upgrade audit above.
