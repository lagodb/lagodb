...
bool
lakebase_parse_vacuum_full(VacuumStmt *stmt, VacuumParams *params)
{
    bool analyze = false;
        ...
        else if (strcmp(opt->defname, "index_cleanup") == 0)
        {
            ...
            else
            {
                ...
                if (pg_strcasecmp(value, "auto") == 0)
                    ...
                    params->index_cleanup = lakebase_vacoptval_from_boolean(opt);
            }
        }
            ...
            params->truncate = lakebase_vacoptval_from_boolean(opt);
...
static List *
lakebase_expand_one(VacuumRelation *vrel, MemoryContext context, bits32 options)
{
    List *result = NIL;
...
List *
lakebase_expand_vacuum_relations(VacuumStmt *stmt, VacuumParams *params,
    ...
    if (stmt->rels != NIL)
    {
        foreach(cell, stmt->rels)
            result = list_concat(result,
                                 lakebase_expand_one(lfirst_node(VacuumRelation, cell),
...
Oid
lakebase_relation_access_method(Oid relid)
{
...
int
lakebase_vacuum_provider_relation(VacuumRelation *vrel, VacuumParams *params,
