...
//! `tuple_fetch_row_version`. Lakebase already has the complete OLD/NEW row in
...
/// 47-bit physical identities used by Lakebase providers. Tokens are
        ...
        NEXT_TRIGGER_ROW_TOKEN.with(|next| {
            ...
            if token >= MAX_TRIGGER_ROW_TOKEN {
                return Err(PgReportError::from_message(
                    ...
                    "Lakebase AFTER-trigger row identity space was exhausted",
                ));
            }
            next.set(token + 1);
