...
pub(super) struct ResultRelationState<P: LakebaseCustomModifyProvider> {
...
impl<P: LakebaseCustomModifyProvider> ResultRelationState<P> {
    unsafe fn new(
        ...
        if !trigger_desc.is_null() {
            ...
            if has_deferred_row_trigger {
                return Err(feature_not_supported(
                    "Lakebase does not support deferrable AFTER ROW triggers; \
...
pub(super) struct ModifyNodeState<P: LakebaseCustomModifyProvider> {
    query_state:
...
impl<P: LakebaseCustomModifyProvider> ModifyNodeState<P> {
    /// # Safety
    ///
...
impl<P: LakebaseCustomModifyProvider> Drop for ModifyNodeState<P> {
    fn drop(&mut self) {
        self.abort();
    }
}
