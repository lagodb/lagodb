//! Join-parameterized CustomPath candidate enumeration and PPI resolution.
...
pub(super) struct ResolvedParamPath {
    pub(super) param_info: *mut pg_sys::ParamPathInfo,
    pub(super) split: PlanPushdownSplit,
}

#[derive(Clone, Copy)]
pub(super) struct ParamPathResolver {
    root: *mut pg_sys::PlannerInfo,
    rel: *mut pg_sys::RelOptInfo,
    provider: &'static dyn ErasedProvider,
}

impl ParamPathResolver {
    pub(super) fn new(
        root: *mut pg_sys::PlannerInfo,
        rel: *mut pg_sys::RelOptInfo,
        provider: &'static dyn ErasedProvider,
    ...
    pub(super) unsafe fn resolve_and_split(
        ...
        let split = unsafe {
            ProviderClauseSplitter::new(self.root, self.rel, self.provider)
                .split(ppi_clauses, ScanClauseSource::Movable)
        };

        ResolvedParamPath { param_info, split }
    }
}

/// Join-parameterized group; PG owns `outer_relids` and `param_info`.
pub struct ParamPathGroup {
    pub outer_relids: *mut pg_sys::Bitmapset,
    pub param_info: *mut pg_sys::ParamPathInfo,
    pub ppi_split: PlanPushdownSplit,
}

impl core::fmt::Debug for ParamPathGroup {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("ParamPathGroup")
            .field("outer_relids", &(self.outer_relids as *const ()))
            .field("param_info", &(self.param_info as *const ()))
            .field("ppi_split.pushed.len", &self.ppi_split.pushed.len())
            .field("ppi_split.residual.len", &self.ppi_split.residual.len())
            .field("ppi_split.recheck.len", &self.ppi_split.recheck.len())
            .finish()
    }
}
...
/// # Safety
///
/// `root`, `rel`, and `joininfo` must belong to the same live planner context,
/// and `provider` must remain registered for the duration of planning.
pub unsafe fn enumerate_param_path_groups(
    root: *mut pg_sys::PlannerInfo,
    rel: *mut pg_sys::RelOptInfo,
    joininfo: *mut pg_sys::List,
    provider: &'static dyn ErasedProvider,
) -> Vec<ParamPathGroup> {
    unsafe { ParamPathEnumerator::new(root, rel, provider).enumerate(joininfo) }
}

#[derive(Clone, Copy)]
pub(super) struct ParamPathEnumerator {
    root: *mut pg_sys::PlannerInfo,
    rel: *mut pg_sys::RelOptInfo,
    provider: &'static dyn ErasedProvider,
}

impl ParamPathEnumerator {
    pub(super) fn new(
        root: *mut pg_sys::PlannerInfo,
        rel: *mut pg_sys::RelOptInfo,
        provider: &'static dyn ErasedProvider,
    ...
    pub(super) unsafe fn enumerate(
        ...
        debug_assert!(
            !self.rel.is_null(),
            "enumerate_param_path_groups: rel must be non-null"
        );
        debug_assert!(
            !self.root.is_null(),
            "enumerate_param_path_groups: root must be non-null"
        );

        let lateral_relids = unsafe { (*self.rel).lateral_relids };
        let rel_relids = unsafe { (*self.rel).relids };

        let mut candidates = unsafe { ParamPathCandidateSet::new(self.rel) };
        unsafe {
        ...
        let mut groups: Vec<ParamPathGroup> = Vec::with_capacity(candidates.len());
        let resolver = ParamPathResolver::new(self.root, self.rel, self.provider);

        for required_outer in candidates {
            debug_assert!(
                unsafe { pg_sys::bms_is_subset(lateral_relids, required_outer) },
                "enumerate_param_path_groups: required_outer must be a superset of lateral_relids"
            );
            debug_assert!(
                !unsafe { pg_sys::bms_overlap(rel_relids, required_outer) },
                "enumerate_param_path_groups: required_outer must not overlap rel->relids"
            );
            debug_assert!(
                !required_outer.is_null(),
                "enumerate_param_path_groups: required_outer for a JoinParameterized variant must be non-empty (PG17 represents empty Bitmapsets as NULL)"
            );

            let resolved = unsafe { resolver.resolve_and_split(required_outer) };
            groups.push(ParamPathGroup {
                outer_relids: required_outer,
                param_info: resolved.param_info,
                ppi_split: resolved.split,
            });
        }
