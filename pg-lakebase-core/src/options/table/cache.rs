...
impl AmCache {
    ...
    /// The returned reference is valid as long as the relation lock is held
    /// and no cache invalidation occurs. Postgres manages the `rd_amcache`
    /// lifetime: on invalidation it calls `pfree` and sets the pointer to NULL,
    /// so the next call will re-populate the cache.
    pub fn get<'a, T: AmCacheable>(
        ...
        // SAFETY: rel_ptr is non-null and points at a locked RelationData.
        unsafe {
