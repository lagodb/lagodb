...
pub(crate) unsafe fn decode_modify_private<P: FdwModify>(
    ...
    let mut reader = unsafe { ForeignPrivateReader::checked_from_list(raw, 0) }?;
    ...
    let operation = ForeignModifyOperation::from_pg(reader.read_i32()? as u32)?;
    let returned_identity = ForeignReturnedIdentity::from_wire(reader.read_i32()?)?;
    let returned_item_pointer_required = reader.read_bool()?;
    if returned_item_pointer_required && !returned_identity.supports_item_pointer() {
        return Err(ForeignModifyError::framework(
            "FDW modify private data requires an undeclared returned identity",
        ));
    }
    let mut updated_columns_reader = reader.read_nested()?;
    let mut updated_columns = Vec::with_capacity(updated_columns_reader.remaining());
    while updated_columns_reader.remaining() > 0 {
        let attno = pg_sys::AttrNumber::try_from(updated_columns_reader.read_i32()?)
            .map_err(|_| {
    ...
    let private_data = unsafe { P::ModifyPrivateData::decode(&mut payload) }?;
    payload.finish()?;
    reader.finish()?;
