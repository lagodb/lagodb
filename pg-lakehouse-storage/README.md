# pg-lakehouse-storage

Centralized storage caching server and client for `pg-lakehouse`.

## Overview

`pg-lakehouse-storage` provides a unified interface for object storage I/O with an integrated block-level cache. It is designed to be used by the `pg-am-iceberg` extension to improve performance and decouple storage logic from PostgreSQL worker processes.
...
- **Zero-Copy & Alignment**: 
  - Supports **O_DIRECT** to bypass the OS Page Cache, preventing double-buffering.
...
### Building
```bash
cargo build -p pg-lakehouse-storage
```
...
## Integration
The client will be used in `pg-am-iceberg/src/storage/object.rs` to replace direct object storage interaction.
