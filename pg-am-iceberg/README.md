...
`pg-am-iceberg` is a PostgreSQL extension implemented in Rust that provides native support for the [Apache Iceberg](https://iceberg.apache.org/) table format. It is built using the [pg-lakehouse-core](https://github.com/robertmu/pg-lakehouse) framework and leverages [pgrx](https://github.com/tcdi/pgrx) for deep integration with PostgreSQL's internal engine.
...
`pg-am-iceberg` allows PostgreSQL to treat Apache Iceberg tables as native tables. Unlike Foreign Data Wrappers (FDW), which operate at the query planning level, `pg-am-iceberg` implements the **Table Access Method (TAM)** interface, providing tighter integration with the storage engine, transaction management, and recovery systems.
...
`pg-am-iceberg` is split into several logical components:
...
```sql
CREATE EXTENSION pg_am_iceberg;
